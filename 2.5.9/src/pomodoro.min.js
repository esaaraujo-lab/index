      ...
      subFolders.forEach(folder=>{
        ...
        requestListPath(fpath,{},function(folderData){
          ...
          if(done===subFolders.length){
            ...
            // ── Detect duplicate names and replace with folder name + index ──
            if(videos.length>1){
              const nameCounts={};
              videos.forEach(v=>{ nameCounts[v.name]=(nameCounts[v.name]||0)+1; });
              const maxCount=Math.max(...Object.values(nameCounts));
              // If the most repeated name appears in >50% of entries, use folder names
              if(maxCount/videos.length>0.3){
                videos.forEach((v,idx)=>{
                  const num=String(idx+1).padStart(3,'0');
                  v.name=num+' - '+(v.folderLabel||allVideos[idx]?.folderName||v.name);
                });
              }
            }
            resolve(videos);
...
`;
  ...
  function syncToPlayer(videoEl){
    ...
    videoEl.addEventListener('play',()=>{
      if(firstPlay){
        firstPlay=false;
        // Open panel on first play so user sees the timer
        panelOpen=true;
        document.getElementById('gdi-pom-panel').classList.add('open');
      }

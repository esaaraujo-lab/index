...
if(p){
  if(window.Plyr||window.videojs||window.DPlayer||window.jwplayer){initPlayerEvents();enableAutoplayWithUnmute();}
  else{const h=document.createElement("script");h.src=p;h.onload=()=>{initPlayerEvents();enableAutoplayWithUnmute();};document.head.appendChild(h);}
}
      ...
      subFolders.forEach(folder=>{
        ...
        requestListPath(fpath,{},function(folderData){
          ...
          if(done===subFolders.length){
            ...
            // ── Detect duplicate names → replace with folder name + sequential index ──
            if(videos.length>1){
              const nameCounts={};
              videos.forEach(v=>{ nameCounts[v.name]=(nameCounts[v.name]||0)+1; });
              const maxCount=Math.max(...Object.values(nameCounts));
              // If any name repeats in >30% of entries, use folder names for ALL
              if(maxCount/videos.length>0.3){
                allVideos.forEach((item,idx)=>{
                  const num=String(idx+1).padStart(3,'0');
                  // item.video is the same object reference as videos[idx]
...
function file_audio(i,e,t,n,a,c,l){const UI=window.UI||{};l=l||[{name:i,url:n,cover:UI.audioposter}];const d=UI.disable_player?"":'<div id="aplayer-container" style="max-width:680px;margin:0 auto;"></div>',o=UI.disable_audio_download?"":renderDownloadButtons(n,e,{showMedia:!0});if($("#content").html(_viewerCard('<i class="bi bi-music-note-beamed gdi-icon-audio"></i>',i,t,d,o)),UI.disable_player)return;const s=document.createElement("link");s.rel="stylesheet",s.href="https://cdn.jsdelivr.net/npm/aplayer@1.10.1/dist/APlayer.min.css",document.head.appendChild(s);const r=document.createElement("script");if(r.src="https://cdn.jsdelivr.net/npm/aplayer@1.10.1/dist/APlayer.min.js",r.onload=function(){window._gdiAPlayer=new APlayer({container:document.getElementById("aplayer-container"),mini:!1,autoplay:!1,theme:"#4d9fec",loop:"all",order:"list",preload:"auto",volume:.7,listFolded:!1,audio:l})},document.head.appendChild(r),l.length<=1){const p=window.location.pathname.split("/").slice(0,-1).join("/")+"/";requestListPath(p,{},function(g){if(!g||!g.data||!g.data.files)return;const f=g.data.files.filter(u=>FILE_TYPES.audio.includes(u.fileExtension));if(f.length>1){const u=f.map(m=>({name:m.name,url:UI.second_domain_for_dl?UI.downloaddomain+m.link:window.location.origin+m.link,cover:UI.audioposter}));window._gdiAPlayer&&window._gdiAPlayer.destroy(),window._gdiAPlayer=new APlayer({container:document.getElementById("aplayer-container"),mini:!1,loop:"all",order:"list",preload:"auto",volume:.7,audio:u});const h=u.findIndex(m=>m.url===n);h>0&&window._gdiAPlayer.list.switch(h)}},null)}
  // ── Sleep mode: escurece a tela gradualmente quando o áudio toca ──
  initAudioSleepMode();}
...
function initAudioSleepMode(){
  ...
  const FADE_DELAY=8000;   // 8s de silêncio → começa a escurecer
  const FADE_TARGET=0.97;  // quase totalmente preto
  ...
  // Clique no overlay ou no botão acorda a tela
  overlay.addEventListener('click', exitSleep);
  btn.addEventListener('click', ()=>{ sleeping ? exitSleep() : enterSleep(); });

  // Movimento do mouse ou teclado acorda a tela temporariamente
  ['mousemove','keydown','touchstart'].forEach(ev=>{
    document.addEventListener(ev, ()=>{
      if(sleeping){
        exitSleep();
        // Dorme de novo depois de 8s sem interação se estiver tocando
        if(window._gdiAPlayer && !window._gdiAPlayer.audio.paused) scheduleSleep();
      }
    }, {passive:true});
  });

  // Aguarda APlayer estar pronto e se conecta aos eventos
  ...
  // APlayer pode não estar pronto ainda — tenta até 5s
  let tries=0;
...
// ============================================================
;(function(){
  // ── Guard: only one instance ever, survives SPA navigation ──
...
`;
  ...
  // ── Flash overlay (outside pom root, survives redraws) ──
  const flashEl = document.createElement('div');
  flashEl.id = 'gdi-pom-flash';
  document.body.appendChild(flashEl);

  // ── Root container (never removed from DOM) ──
  ...
  function playWarningBeep(secondsLeft){
    // Urgent triple tick in last 10s
  ...
  function updateUI(){
    ...
    // Browser title badge while running
    if(running){
  ...
  // ── Player sync ──
  // We hook into the GDI player via MutationObserver + polling
  // Works for all player types (Plyr, VideoJS, DPlayer, native)
  function getVideoEl(){
  ...
  // Watch for video element appearing (SPA navigation creates it dynamically)
  const observer=new MutationObserver(()=>{
    const vid=getVideoEl();
    if(vid&&!playerSynced) syncToPlayer(vid);
    // If video was removed (page nav), reset sync flag
  ...
  // GDI's init() calls $("body").html(...) which wipes children.
  // We re-append our root + flash whenever they get detached.
  const domGuard=new MutationObserver(()=>{
    if(!document.body.contains(root)){
      document.body.appendChild(root);
      playerSynced=false;
      // Ensure panel stays closed on re-inject
      panelOpen=false;
      document.getElementById('gdi-pom-panel').classList.remove('open');
      updateUI();
    }

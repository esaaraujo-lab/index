...
console.log('[GDI app] build v11-fixedlayout');

// ── v11.1: corrige vazamento de largura (flexbox min-width:auto) ──
// Filhos de .gdi-study-left (flex column) e a URL de download dentro de
// .gdi-dl-url-row (flex row) tinham min-width:auto e se recusavam a encolher,
// empurrando a caixa além da largura do vídeo. Trava a cadeia inteira e
// trunca a URL com reticências.
...
// Lista TODAS as páginas de uma pasta — fetch direto, nunca lança.
async function gdiListAllFiles(path,pw,onPage){
  ...
  if(window.__GDI_VERBOSE)console.log('[GDI Playlist] listado',out.length,'itens de',path);
  return out;
...
function gdiResetStudyModeClasses(){
  document.body.classList.remove('gdi-fv','gdi-fm','gdi-nomat','gdi-lock');
  document.documentElement.classList.remove('gdi-lock');
}

function render(i){gdiResetStudyModeClasses();i.indexOf("?")>=0&&(i=i.substr(0,i.indexOf("?"))),title(i),nav(i);const e=/\/\d+:$/g;if(i.includes("/fallback")){window.scroll_status={event_bound:!1,loading_lock:!1};const t=getQueryVariable("a"),n=decodeURIComponent(getQueryVariable("id")||"");return t?fallback(n,!0):list(null,n,!0)}else window.MODEL?.is_search_page?(window.scroll_status={event_bound:!1,loading_lock:!1},render_search_result_list()):i.match(e)||i.slice(-1)=="/"?(window.scroll_status={event_bound:!1,loading_lock:!1},list(i)):file(i)}
...
</div>`}}if(l&&UI.allow_selecting_files&&(document.getElementById("select_items").style.display="flex"),t.html((t.data("curPageIndex")=="0"?"":t.html())+a),n){const d=formatFileSize(c)||"0 Bytes",o=t.find(".countitems").length,s=t.find(".size_items").length,r=o===0?"Empty folder":`${o} item${o===1?"":"s"}`,p=s>0?` &middot; ${s} file${r===1?"":"s"}, ${d}`:"";$("#count").addClass("show").html(r+p)}}catch(t){console.error(t)}}
...
function append_search_result_to_list(i){const UI=window.UI||{};try{const e=$("#list"),t=e.data("nextPageToken")===null;let n="",a=0,c=!1;for(const l in i){const d=i[l];d.size==null&&(d.size=""),d.modifiedTime=utc2delhi(d.modifiedTime);const o=typeof d.rootIdx=="number"?d.rootIdx:-1;if(d.mimeType=="application/vnd.google-apps.folder")n+=`<a style="cursor:pointer;" onclick="onSearchResultItemClick('${escJs(d.id)}', false, ${o})" data-bs-toggle="modal" data-bs-target="#SearchModel"
  ...
  <span class="gdi-row-name" onclick="onSearchResultItemClick('${escJs(d.id)}', true, ${o})" data-bs-toggle="modal" data-bs-target="#SearchModel" style="cursor:pointer;" data-size="${UI.display_size?d.size:""}">${escHtml(d.name)}</span>
...
.gdi-study{max-width:1600px;margin:0 auto;padding:10px 14px 24px;box-sizing:border-box;}
...
.gdi-study-grid{display:grid;grid-template-columns:minmax(0,58fr) minmax(0,42fr);gap:14px;align-items:start;width:100%;}
...
.gdi-file-header-name{font-weight:600;font-size:15px;color:#f0f6fc;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
.gdi-file-header-meta{font-size:12px;color:#8b949e;}

/* ── Trava de rolagem do body enquanto o overlay de tela cheia está ativo ── */
html.gdi-lock,body.gdi-lock{overflow:hidden!important;height:100%;}

/* ── Modos de tela cheia (Foco na aula / Foco no material) ──
   Usamos flexbox para centralizar em vez de margin:auto sobre position:fixed,
   que não centraliza de forma confiável e gera o efeito de "janela deslocada"
   com barra de rolagem extra. */
body.gdi-fv .gdi-study,body.gdi-fm .gdi-study{
  position:fixed;top:0;left:0;right:0;bottom:0;
  margin:0;max-width:none;width:auto;
  z-index:8500;background:#0b0e14;
...
body.gdi-fv .gdi-study-grid{grid-template-columns:1fr!important;}
body.gdi-fv .gdi-study-right{display:none;}
body.gdi-fm .gdi-study-left{display:none;}
body.gdi-fm .gdi-study-grid{grid-template-columns:1fr!important;}
@media(max-width:980px){.gdi-study-grid{grid-template-columns:1fr;}}

/* ── Sem material (nenhum PDF encontrado) — coluna única centralizada
   usando a mesma largura do vídeo, em qualquer modo (normal ou tela cheia) ── */
...
body.gdi-fv .gdi-mat-body{height:auto;}
body.gdi-fm .gdi-mat-body{height:calc(100dvh - 130px);}
...
.gdi-notes-head{display:flex;justify-content:space-between;align-items:center;font-size:13px;color:#e6edf3;margin-bottom:6px;}
...
// ── Modos de layout (toggle) + zoom automático no Foco no Material ──
...
function gdiSetMode(m){
  ...
  // Sem painel de materiais: mantém alinhamento central em qualquer modo
  if(!document.querySelector('.gdi-study-right'))document.body.classList.add('gdi-nomat');
...
// Toggle: clicar de novo no modo ativo sai dele (útil quando não há "Dividido")
document.querySelectorAll('.gdi-mode-btn[data-mode]').forEach(b=>{
  b.addEventListener('click',()=>{
    const m=b.dataset.mode;
    const cls=m==='fv'?'gdi-fv':'gdi-fm';
    const wasActive=document.body.classList.contains(cls);
    gdiSetMode(wasActive?'split':m);
  });
});
...
async function loadCrossFolderPlaylist(onProgress){
  ...
  const parentPw=localStorage.getItem('password'+parentPath)||'';
  ...
  async function scanBatch(count){
    for(let k=0;k<count&&cursor<subFolders.length;k++){
      ...
      const fpw=localStorage.getItem('password'+fpath)||'';
...
try{
  const folderPw=localStorage.getItem('password'+folderPath)||'';
...
// ─────────────────────────────────────────────────────────────
// MATERIAIS — v11: sem PDF → remove Dividido e Foco no material,
// classe gdi-nomat = layout centralizado com a largura do vídeo.
// Mobile: botão "Abrir material" (sem iframe).
// ─────────────────────────────────────────────────────────────
...
async function gdiFindPdfs(){
  const isPdf=x=>(x.fileExtension||'').toLowerCase()==='pdf'||/pdf/i.test(x.mimeType||'');
  const pw0=localStorage.getItem('password'+folderPath)||'';
  const here=await gdiListAllFiles(folderPath,pw0);
  let found=here.filter(isPdf);
  if(!found.length){
    const subs=here.filter(x=>x.mimeType==='application/vnd.google-apps.folder').slice(0,20);
    for(const sf of subs){
      const fp=folderPath+encodeURIComponent(sf.name)+'/';
      const pww=localStorage.getItem('password'+fp)||'';
      found=found.concat((await gdiListAllFiles(fp,pww)).filter(isPdf));
      if(found.length)break;
    }
  }
  if(!found.length){
    const pw1=localStorage.getItem('password'+parentPath)||'';
    found=(await gdiListAllFiles(parentPath,pw1)).filter(isPdf);
  }
  const seen=new Set();const out=[];
  found.forEach(x=>{if(!seen.has(x.name)){seen.add(x.name);out.push(x)}});
  return out.slice(0,12);
}
function gdiHideMaterialsPanel(){
  // Remove painel de materiais
  document.querySelector('.gdi-study-right')?.remove();
  // Remove "Dividido" e "Foco no material" — fica só "Foco na aula" + Assistido
  document.querySelector('.gdi-mode-btn[data-mode="split"]')?.remove();
  document.querySelector('.gdi-mode-btn[data-mode="fm"]')?.remove();
  // Layout centralizado: coluna única com a largura do vídeo
  document.body.classList.add('gdi-nomat');
  // Se estava em "Foco no material", sai do modo
  if(document.body.classList.contains('gdi-fm')){
    gdiSetMode('split');
  }
  document.querySelectorAll('.gdi-mode-btn[data-mode]').forEach(b=>{
    if(b.dataset.mode!=='fv')b.classList.remove('active');
  });
}
(async function buildMaterials(){
  ...
  try{
    const pdfs=await gdiFindPdfs();
    if(!pdfs.length){
      gdiHideMaterialsPanel();
      console.log('[GDI Materiais] nenhum PDF — layout centralizado, s\u00f3 Foco na aula + Assistido');
      return;
    }
    const items=pdfs.map(x=>{
      const cls=gdiClassifyPdf(x.name);
      const base=UI.second_domain_for_dl?UI.downloaddomain+x.link:window.location.origin+x.link;
      const url=base+(x.link.includes('?')?'&':'?')+'inline=true';
      return{name:x.name,label:cls.l,icon:cls.i,ord:cls.ord,url};
    });
    items.sort((x,y)=>x.ord-y.ord||x.name.localeCompare(y.name,undefined,{numeric:true}));
    const used={};
    ...
    function show(idx){
      ...
      if(isMobile){
        // Android/iOS: sem leitor de PDF em iframe — botão abre no leitor do dispositivo
      ...
      if(!frames[idx]){
        ...
        bodyEl.innerHTML='';bodyEl.appendChild(frames[idx]);
      }
    }
    ...
    show(0);
    console.log('[GDI Materiais]',items.length,'PDFs:',items.map(x=>x.tabLabel).join(' | '));
  }catch(err){
    gdiHideMaterialsPanel();
  }
})();
...
// ─────────────────────────────────────────────────────────────
// ASSISTIDO + ANOTAÇÕES (salvos no JSON do usuário no Drive)
// ─────────────────────────────────────────────────────────────
...
// ── Resume + marcação automática de assistido ──
(function(){let n=0;const t=setInterval(()=>{
  const v=document.querySelector('video');
  if(v){clearInterval(t);
    attachResumeTracking(v,()=>{
    ...
    v.addEventListener('timeupdate',()=>{
      const el=document.getElementById('gdi-note-time');
      if(el)el.textContent=gdiFmtTime(v.currentTime);
    });
...
// ============================================================
// RESUME + ESTADO DO USUÁRIO NO DRIVE
// ============================================================
const GDIResume=(()=>{
  const KEY='gdi-resume-v2',MAX=300,MIN_SAVE=5,END_MARGIN=15;
  ...
  return{
    MIN_SAVE,
    get:p=>all()[norm(p)]||null,
    set(p,t,d){
      p=norm(p); if(!p)return;
      const tt=Math.floor(Number(t)||0);
      if(d&&tt>d-END_MARGIN){this.del(p);return}
      if(tt<MIN_SAVE)return;
      const m=all();
      m[p]={t:tt,d:Math.floor(d||0),at:Date.now()};
      const ks=Object.keys(m);
      if(ks.length>MAX)ks.sort((a,b)=>m[a].at-m[b].at).slice(0,ks.length-MAX).forEach(k=>delete m[k]);
      write(m);
    },
    del(p){p=norm(p);const m=all();delete m[p];write(m)}
  };
})();

function gdiFmtTime(s){s=Math.floor(s||0);return Math.floor(s/60)+':'+String(s%60).padStart(2,'0')}

// Estado do usuário: JSON por usuário no Drive (via /userstate do Worker)
const GDIUser=(()=>{
  const EMPTY={watched:{},last:null,resume:{},notes:{}};
  ...
  async function _load(){
    ...
    state.watched=state.watched||{};state.resume=state.resume||{};state.notes=state.notes||{};
  ...
  return{
    ready:_load,
    markWatched:p=>mut(s=>{s.watched[p]={at:Date.now()}}),
    unmarkWatched:p=>mut(s=>{delete s.watched[p]}),
    isWatched:p=>!!(loaded&&state.watched&&state.watched[p]),
    setLast:p=>mut(s=>{s.last={path:p,at:Date.now()}}),
    setResume:(p,t,d)=>mut(s=>{s.resume[p]={t:Math.floor(t),d:Math.floor(d||0),at:Date.now()}}),
    getResume:p=>(loaded&&state.resume&&state.resume[p])||null,
    addNote:(p,t,text)=>mut(s=>{s.notes[p]=s.notes[p]||[];s.notes[p].push({t:Math.floor(t||0),text:String(text).slice(0,2000),at:Date.now()})}),
    delNote:(p,i)=>mut(s=>{if(s.notes[p])s.notes[p].splice(i,1)}),
    getNotes:p=>(loaded&&state.notes&&state.notes[p])||[],
    flush:_flush
  };
})();
GDIUser.ready();
...
function attachResumeTracking(media,getKey){
  ...
  let lastSave=0,appliedKey=null,tries=0,loggedEmpty=false;
  ...
  const save=()=>{const k=key();if(!k)return;
    GDIResume.set(k,media.currentTime,dur());
    GDIUser.setResume(k,media.currentTime,dur());};

  function applySeek(reason){
    const k=key(); if(!k||appliedKey===k)return;
    const sv=GDIUser.getResume(k)||GDIResume.get(k);
    if(!sv){
      if(!loggedEmpty){loggedEmpty=true;console.log('[GDI Resume] sem posição salva para',k)}
      return;
    }
    if(sv.t<GDIResume.MIN_SAVE)return;
    if(media.readyState<1)return;
    const d=dur();
    if(d&&sv.t>d-15){GDIResume.del(k);appliedKey=k;return}
    try{
      media.currentTime=sv.t;
      appliedKey=k;
      console.log('[GDI Resume] retomando em',gdiFmtTime(sv.t),'via',reason);
      showResumeToast(sv.t,()=>{try{media.currentTime=0}catch(e){}GDIResume.del(k)});
    }catch(e){}
  }

  media.addEventListener('timeupdate',()=>{const n=Date.now();if(n-lastSave>5000){lastSave=n;save()}});
  media.addEventListener('pause',save);
  media.addEventListener('ended',()=>GDIResume.del(key()));
  ['loadedmetadata','canplay','durationchange','playing'].forEach(ev=>media.addEventListener(ev,()=>applySeek(ev)));
  document.addEventListener('visibilitychange',()=>{if(document.hidden)save()});
  window.addEventListener('beforeunload',save);
  const poll=setInterval(()=>{
    if(appliedKey||++tries>20){clearInterval(poll);return}
    applySeek('poll');
  },300);
  applySeek('attach');
}
...
// ============================================================
// MODO DESCANSO — áudio: automático + manual | vídeo: só manual
// ============================================================
function initSleepMode(type){
  ...
  if(type==='audio'){
    ...
    const w=setInterval(()=>{
      const v=document.querySelector('video');
      if(v){clearInterval(w);v.addEventListener('ended',exitSleep);}
...
// ============================================================
// POMODORO v2.2 — só o 🍅; manual; SÓ APARECE com vídeo/áudio
// ============================================================
;(function(){
  function gdiPomBoot(){
...
`;
    ...
    // Esconde de imediato se a página não tem mídia (evita piscar no load)
    ...
    // Guarda DOM: sobrevive ao $("body").html() do init()
    new MutationObserver(()=>{
      if(!document.body.contains(root))document.body.appendChild(root);
      if(!document.body.contains(flashEl))document.body.appendChild(flashEl);
    }).observe(document.body,{childList:true});

    // 🍅 só aparece com vídeo/áudio na página (como o modo descanso)
    setInterval(()=>{
    ...
    updateUI();
    console.log('[GDI Pomodoro] v2.2 pronto (manual, s\u00f3 com m\u00eddia)');
  }

...
console.log('[GDI app] build v8-nomob');
...
body.gdi-fv .gdi-study-grid{grid-template-columns:1fr;}
body.gdi-fv .gdi-study-right{display:none;}
body.gdi-fm .gdi-study-left{display:none;}
body.gdi-fm .gdi-study-grid{grid-template-columns:1fr;}
...
// ── Modos de layout + zoom automático no Foco no Material ──
...
// ─────────────────────────────────────────────────────────────
// MATERIAIS — abas só se houver PDF na pasta da aula, 1 nível
// abaixo ou pasta da disciplina. Sem PDF: coluna única, sem botões
// de foco. Mobile: botão "Abrir material" (sem iframe).
// ─────────────────────────────────────────────────────────────
...
function gdiHideMaterialsPanel(){
  ...
  document.querySelectorAll('.gdi-mode-btn[data-mode="fv"],.gdi-mode-btn[data-mode="fm"]').forEach(b=>b.remove());
  document.body.classList.remove('gdi-fv','gdi-fm');
  try{localStorage.removeItem('gdi-study-mode')}catch(_){}
}
(async function buildMaterials(){
  ...
  try{
    ...
    if(!pdfs.length){
      ...
      console.log('[GDI Materiais] nenhum PDF — layout de coluna \u00fanica');
      return;
    }
...
// ============================================================
// POMODORO v2.1 — só o 🍅; nada automático; só roda se você clicar
// ============================================================
;(function(){
  function gdiPomBoot(){
...
`;
    ...
    updateUI();
    console.log('[GDI Pomodoro] v2.1 pronto (manual)');
  }

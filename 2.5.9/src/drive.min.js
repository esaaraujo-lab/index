...
// ─────────────────────────────────────────────────────────────
// Lista TODAS as páginas de uma pasta — fetch DIRETO ao endpoint
// da pasta. NÃO passa pelo requestListPath (suas flags posicionais
// de fallback/quiet causavam POST em /0:fallback com id vazio →
// 400 → timeout → lista vazia). Timeout 30s por página.
// ─────────────────────────────────────────────────────────────
async function gdiListAllFiles(path,pw,onPage){
  ...
  for(let guard=0;guard<50;guard++){
    ...
    if(!files){console.warn('[GDI Playlist] pág',idx+1,'falhou em',path);break;}
    out.push(...files);
    console.log('[GDI Playlist] pág',idx+1,':',files.length,'itens, próxima:',!!page.nextPageToken);

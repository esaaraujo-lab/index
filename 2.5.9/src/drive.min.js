...
console.log('[GDI app] build v3-cross');
...
// ─────────────────────────────────────────────────────────────
// Lista TODAS as páginas de uma pasta. Log por página + callback
// de progresso. Resolve SEMPRE (timeout 30s por página).
// ─────────────────────────────────────────────────────────────
async function gdiListAllFiles(path,pw,onPage){
  let out=[],token='',idx=0;
  const shortName=()=>{try{const parts=decodeURIComponent(path).split('/').filter(Boolean);return parts[parts.length-1]||path}catch(_){return path}};
    ...
    const page=await new Promise(res=>{
      ...
      try{
        requestListPath(path,{password:pw||'',page_token:token,page_index:idx},function(d){
          finish({files:(d&&d.data&&Array.isArray(d.data.files))?d.data.files:[],next:(d&&d.nextPageToken)?d.nextPageToken:''});
        },null,2,true);
      }catch(_){finish(null)}
      setTimeout(()=>finish(null),30000);
    });
    if(!page||!Array.isArray(page.files)){console.warn('[GDI Playlist] página',idx+1,'falhou/timeout em',shortName());break}
    out=out.concat(page.files);
    console.log('[GDI Playlist] pág',idx+1,'de',shortName()+':',page.files.length,'itens, próxima:',!!page.next);
...
<div id="gdi-playlist-wrap" style="margin-top:16px;display:none;">
  ...
  <div id="gdi-playlist-list" style="max-height:300px;overflow-y:auto;border:1px solid rgba(255,255,255,0.12);border-radius:8px;padding:6px;background:rgba(0,0,0,0.2);"></div>
</div>`;
...
// ─────────────────────────────────────────────────────────────
// PLAYLIST ANINHADA: TODOS os vídeos de TODAS as subpastas irmãs.
// Varredura com 4 em paralelo + render progressivo via onProgress.
// ─────────────────────────────────────────────────────────────
async function loadCrossFolderPlaylist(onProgress){
  ...
  const collected=[];let doneCount=0;
  let cursor=0;
  async function worker(){
    while(cursor<subFolders.length){
      ...
      doneCount++;
      if(onProgress)try{onProgress(collected.slice(),doneCount,subFolders.length)}catch(_){}
    }
  }
  ...
  console.log('[GDI Playlist] aninhada: total',collected.length,'vídeos em',doneCount,'subpastas');
...
(async function buildPlaylist(){
  ...
  // Aninhada: mostra progresso na tela conforme subpastas são varridas
  const crossVideos=await loadCrossFolderPlaylist(function(partial,done,total){
    if(partial.length<2)return;
    playlistVideos=dedupePlaylist(partial);
    const ci=playlistVideos.findIndex(v=>(gdiNormPath(v.folder)===gdiNormPath(folderPath)&&v.name===i)||v.streamUrl===a);
    if(ci!==-1){currentIndex=ci;renderPlaylistUI();}
    const cnt=document.getElementById('gdi-playlist-count');
    if(cnt)cnt.textContent=`varrendo ${done}/${total}\u2026 ${playlistVideos.length} aulas`;
  });
  if(crossVideos.length>1){
    ...
    console.log('[GDI Playlist] aninhada FINAL:',playlistVideos.length,'itens, currentIndex=',currentIndex);
    if(currentIndex===-1){
      playlistVideos.unshift({name:i,size:t,streamUrl:a,pageUrl:window.location.href,mimeType:c,poster:n,folder:folderPath});
      currentIndex=0;
    }

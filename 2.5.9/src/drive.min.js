...
// ── CSS do modo estudo (AlfaCon-like) + ícones translúcidos ──
...
.gdi-study-modes{display:flex;gap:6px;}
...
.gdi-study-head{display:flex;align-items:center;gap:10px;}
.gdi-study-title{min-width:0;}
...
<div class="gdi-study" id="gdi-study">
  <div class="gdi-study-bar">
    ...
    <div class="gdi-study-modes">
      ...
      <button class="gdi-mode-btn" data-mode="fm" title="Foco no material (s\u00f3 PDF)"><i class="bi bi-file-earmark-pdf-fill"></i><span class="d-none d-md-inline">Foco no material</span></button>
    </div>
  </div>
...
async function loadCrossFolderPlaylist(onProgress){
  ...
  if(!subFolders.length){console.log('[GDI Playlist] aninhada: nenhuma subpasta');return [];}
  ...
  console.log('[GDI Playlist] aninhada completa:',collected.length,'v\u00eddeos em',cursor,'subpastas');
...
// ─────────────────────────────────────────────────────────────
// MATERIAIS (PDFs) estilo AlfaCon — busca: pasta da aula →
// subpastas (2 níveis) → pasta da disciplina. Abas temáticas.
// ─────────────────────────────────────────────────────────────
...
async function gdiFindPdfs(){
  ...
  const direct=(await gdiListAllFiles(folderPath,pw0)).filter(isPdf);
  let found=direct;
  if(!found.length){
    const subs=(await gdiListAllFiles(folderPath,pw0)).filter(x=>x.mimeType==='application/vnd.google-apps.folder').slice(0,20);
    for(const sf of subs){
      ...
      if(!found.length){
        const subs2=(await gdiListAllFiles(fp,pww)).filter(x=>x.mimeType==='application/vnd.google-apps.folder').slice(0,15);
        for(const s2 of subs2){
          const fp2=fp+encodeURIComponent(s2.name)+'/';
          const pw2=localStorage.getItem('password'+fp2)||'';
          found=found.concat((await gdiListAllFiles(fp2,pw2)).filter(isPdf));
        }
      }
    }
  }

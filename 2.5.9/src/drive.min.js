...
console.log('[GDI app] build v11-scrollfix');
...
/* v11: modos de foco — SEM overflow:hidden no CSS. O travamento do scroll
   do body é feito via gdiLockScroll() que COMPENSA a largura da scrollbar,
   eliminando o "shift" para a esquerda. O .gdi-study fixed ocupa a viewport
   inteira e se centra com margin:auto normalmente (sem scrollbar concorrente). */
body.gdi-fv .gdi-study,body.gdi-fm .gdi-study{position:fixed;inset:0;z-index:8500;background:#0b0e14;overflow-y:auto;overscroll-behavior:contain;padding:12px 16px 24px;}
...
/* Foco na aula SEM material: vídeo em largura máxima (ignora o .nomat de 960px) */
body.gdi-fv .gdi-study.nomat{max-width:1600px;}
body.gdi-fv .gdi-study.nomat .gdi-study-left{max-width:100%;}
@media(max-width:980px){.gdi-study-grid{grid-template-columns:1fr;}.gdi-study.nomat .gdi-study-left{max-width:100%;}}
...
// ── v11: trava o scroll do body COM pensação da scrollbar (sem shift) ──
let _sbw=0;
function gdiLockScroll(on){
  if(on){
    _sbw=window.innerWidth-document.documentElement.clientWidth;
    document.body.style.overflow='hidden';
    if(_sbw>0)document.body.style.paddingRight=_sbw+'px';
  }else{
...
// ─────────────────────────────────────────────────────────────
// MATERIAIS — v11: sem PDF → .nomat (centrado) + remove Dividido
// e Foco no material (fica ⚡ + ✔) + destrava scroll via gdiLockScroll.
// ─────────────────────────────────────────────────────────────
...
function gdiHideMaterialsPanel(){
  ...
  if(study)study.classList.add('nomat');
...
(async function buildMaterials(){
  ...
  try{
    ...
    if(!pdfs.length){
      ...
      console.log('[GDI Materiais] nenhum PDF — layout centrado, \u26a1 + \u2714 apenas');
      return;
    }
...
`;
    ...
    // 🍅 só aparece com vídeo/áudio na página
    setInterval(()=>{

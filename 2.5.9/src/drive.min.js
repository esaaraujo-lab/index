...
console.log('[GDI app] build v5-progressive');
...
// ─────────────────────────────────────────────────────────────
// Lista TODAS as páginas de uma pasta — fetch DIRETO (v4).
// ─────────────────────────────────────────────────────────────
async function gdiListAllFiles(path,pw,onPage){
  ...
  console.log('[GDI Playlist] listado',out.length,'itens de',path);
  return out;
}
...
// FIX: filtra fragmentos de download (.part-FragXX, KB) e arquivos minúsculos
...
// Ordena e renomeia duplicatas usando origName (idempotente entre lotes)
...
// Localiza o vídeo atual: nome original OU nome renomeado, na pasta dele
function gdiFindCurrent(list,normFn){
  const nf=normFn||(p=>{try{return decodeURIComponent(String(p||'')).replace(/\/+$/,'')}catch(_){return String(p||'').replace(/\/+$/,'')}});
...
// ─────────────────────────────────────────────────────────────
// PLAYLIST ANINHADA PROGRESSIVA:
//   1) Varre subpastas até ~60 aulas → playlist aparece
//   2) Depois, a cada 2s, varre o próximo lote e incrementa na tela
// até esgotar as subpastas. Sem rajada de requisições.
// ─────────────────────────────────────────────────────────────
async function loadCrossFolderPlaylist(onProgress){
  const INITIAL_ITEMS=60;      // carrega ~60 aulas de imediato
  const BATCH_SUBS=3;          // subpastas por lote incremental
  const BATCH_DELAY=2000;      // a cada 2 segundos
  ...
  console.log('[GDI Playlist] aninhada progressiva:',subFolders.length,'subpastas');
  ...
  // 1ª fase: o suficiente para ~60 aulas na tela
  while(cursor<subFolders.length&&collected.length<INITIAL_ITEMS){await scanBatch(5);}
  if(onProgress)try{onProgress(collected.slice(),cursor,subFolders.length)}catch(_){}
  // 2ª fase: lotes pequenos a cada 2s até o fim
...
(async function buildPlaylist(){
  const crossMode=()=>{try{return playlistVideos.length&&playlistVideos[0].folderLabel}catch(_){return false}};
  ...
  // Aninhada: playlist cresce na tela a cada 2s
  const crossVideos=await loadCrossFolderPlaylist(function(partial,done,total){
    ...
    if(crossMode()||true)renderPlaylistUI();

...
console.log('[GDI core] v19.3-completo');
...
// stubs substituídos pelo módulo M1 do gdi-extras.js
...
// ═══ ★ v19.3: cache de playlist multi-pasta ═══
// Ordem: playlist.json do Drive (sonda 1×/sessão) → cache local 7 dias → escanear.
// Correções v19.3:
//  • nomes SEMPRE preenchidos (reconstrói do link se vier vazio)
//  • prefixo da pasta re-aplicado no carregamento do cache (finalize)
//  • cache corrompido (nomes vazios) é descartado → reescaneia sozinho
//  • 401 do playlist.json: no máx. 1× por sessão/pasta (sessionStorage)
function gdiPlCacheKey(p){return 'gdi-xpl::'+(window.location.host||'')+'::'+(p||'/')}
function gdiPlaylistFromJSON(arr){
  const UI=window.UI||{},out=[];
  (arr||[]).forEach(x=>{
    const link=x.l||x.link||'';
    if(!link)return;
    let name=x.n||x.name||'';
    if(!name){
      try{name=decodeURIComponent((link.split('/').pop()||'').split('?')[0])||''}catch(_){name=''}
    }
    const folder=x.fd||x.folder||'';
    out.push({id:x.id||'',name,origName:x.o||x.origName||name,folderLabel:x.f||x.folderLabel||null,
      size:formatFileSize(x.s||x.size||0),sizeBytes:Number(x.s)||0,
      streamUrl:UI.second_domain_for_dl?UI.downloaddomain+link:window.location.origin+link,
      pageUrl:folder+encodeURIComponent(name)+'?a=view',
      mimeType:x.m||x.mimeType||'video/mp4',
      poster:x.t?String(x.t).replace("s220","s0"):UI.poster,
      thumbRaw:x.t||'',rawLink:link,folder});
  });
  return out;
}
// prefixa o nome da pasta em itens duplicados (mesma lógica do escaneamento)
function gdiFinalizeCachedNames(list){
  try{
    list.sort((x,y)=>
    ...
    const counts={};
    list.forEach(x=>{const o2=x.origName||x.name;counts[o2]=(counts[o2]||0)+1});
    list.forEach(x=>{
      const o2=x.origName||x.name;
      if(counts[o2]>1&&x.folderLabel&&x.name===o2)x.name=x.folderLabel+' - '+o2;
    });
  }catch(_){}
}
async function gdiPlaylistCacheGet(parent){
  // 1) playlist.json do Drive — sonda no máx. 1× por sessão/pasta
  const probeKey='gdi-pljson-probe::'+(window.location.host||'')+'::'+(parent||'/');
  let probe=null;try{probe=sessionStorage.getItem(probeKey)}catch(_){}
  if(probe!=='missing'){
    try{
      const ctrl=new AbortController();const to=setTimeout(()=>ctrl.abort(),4000);
      const r=await fetch(parent+'playlist.json',{cache:'no-store',signal:ctrl.signal});
      clearTimeout(to);
      if(r.ok){
        const j=await r.json();
        const arr=Array.isArray(j)?j:(j&&Array.isArray(j.videos)?j.videos:null);
        if(arr){
          const v2=gdiPlaylistFromJSON(arr);
          if(v2.length>1&&!v2.some(x=>!x.name)){
            try{sessionStorage.setItem(probeKey,'ok')}catch(_){}
            gdiFinalizeCachedNames(v2);
            console.log('[GDI Playlist] carregada do playlist.json do Drive ('+v2.length+' aulas)');
            return v2;
          }
        }
      }else{
        try{sessionStorage.setItem(probeKey,'missing')}catch(_){}
      }
    }catch(_){}
  }
  // 2) cache local (7 dias) — com verificação de sanidade
  try{
    const raw=localStorage.getItem(gdiPlCacheKey(parent));
    if(raw){
      const o=JSON.parse(raw);
      if(o&&Array.isArray(o.v)&&o.v.length>1&&Date.now()-(o.at||0)<7*86400000){
        const v2=gdiPlaylistFromJSON(o.v);
        if(v2.length>1&&!v2.some(x=>!x.name)){
          gdiFinalizeCachedNames(v2);
          console.log('[GDI Playlist] carregada do cache local ('+v2.length+' aulas)');
          return v2;
        }
        console.warn('[GDI Playlist] cache local inválido (nomes vazios) — reescaneando');
      }
    }
  }catch(_){}
  return null;
}
function gdiPlaylistCacheSet(parent,list){
  try{
    const v=(list||[]).filter(x=>x.name&&x.rawLink).map(x=>({n:x.name,o:x.origName||x.name,f:x.folderLabel||null,
      s:x.sizeBytes||0,l:x.rawLink||'',m:x.mimeType||'',fd:x.folder||'',t:x.thumbRaw||''}));
    if(v.length>1)localStorage.setItem(gdiPlCacheKey(parent),JSON.stringify({at:Date.now(),v}));
  }catch(_){}
}
// ═══ fim do cache v19.3 ═══
...
// fallback de PDF no core — o M17 (extras) sobrepõe com pdf.js
function file_pdf(i,e,t,n,a,c){
  $("#content").html(_viewerCard('<i class="bi bi-file-earmark-pdf-fill gdi-icon-pdf"></i>',i,t,
    `<iframe src="${n}" style="width:100%;height:calc(100dvh - 220px);min-height:480px;border:0;border-radius:10px;background:#fff;" allow="fullscreen"></iframe>`,
    renderDownloadButtons(n,e)));
}
  ...
  <div class="gdi-study-grid">
    <section class="gdi-study-left">
      ...
      <div id="gdi-playlist-wrap" style="display:none;">
        ...
        <div id="gdi-playlist-list" style="overflow-y:auto;border:1px solid rgba(255,255,255,.12);border-radius:10px;padding:6px;background:rgba(0,0,0,.2);max-height:240px;"></div>
      </div>
    </section>
...
if(p){
  if(window.Plyr||window.videojs||window.DPlayer||window.jwplayer){initPlayerEvents();gdiAnnounceVideoFromDOM();}
  else{const h=document.createElement("script");h.src=p;h.onload=()=>{initPlayerEvents();gdiAnnounceVideoFromDOM();};document.head.appendChild(h);}
}
...
function renderPlaylistUI(){
  ...
  playlistVideos.forEach((m,v)=>{
    ...
    let watched=false;
    if(window.GDIUser&&GDIUser.isWatched){
      const raw=(m.pageUrl||'').split('?')[0];
      try{watched=GDIUser.isWatched(raw)||(window.gdiNormKey&&GDIUser.isWatched(gdiNormKey(raw)));}catch(_){}
    }
...
window.playNextInPlaylist=playNextInPlaylist;
...
function buildPlaylistFromFiles(files,basePath,folderLabel){
  ...
  files.forEach(v=>{
    if(FILE_TYPES.video.includes(v.fileExtension)||v.mimeType&&v.mimeType.includes("video")){
      ...
      videos.push({id:v.id,name:v.name,origName:v.name,folderLabel:folderLabel||null,size:formatFileSize(v.size),sizeBytes:bytes,rawLink:v.link,thumbRaw:v.thumbnailLink||"",streamUrl:y,pageUrl:_,mimeType:v.mimeType,poster:b,folder:basePath});
    }
  });
  return videos;
}
...
try{
  ...
  // ★ v19.3: playlist.json do Drive / cache local antes de escanear
  if(window.location.pathname!=='/fallback'){
    const cachedVids=await gdiPlaylistCacheGet(parentPath);
    if(cachedVids&&cachedVids.length>1){
      playlistVideos=cachedVids;
  ...
  if(crossVideos.length>1){
    ...
    renderPlaylistUI();
    gdiPlaylistCacheSet(parentPath,crossVideos);
    console.log('[GDI Playlist] cache salvo ('+crossVideos.length+' aulas) — pr\u00f3ximas visitas n\u00e3o reescaneiam');
  }
...
// ═══════════════════════════════════════════════════════════════
// CSS GLOBAL (inclui regras de foco como redundância)
// ═══════════════════════════════════════════════════════════════
...
.gdi-study-left{min-width:0;display:flex;flex-direction:column;gap:10px;overflow-x:hidden;}

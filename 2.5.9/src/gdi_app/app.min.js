...
console.log('[GDI core] v19.4-fix (playlist: sem render por lote, teto 600, delegation)');

// ★FIX: collator único e rápido (replaces localeCompare caro, usado em sorts grandes)
const __gdiCollator=new Intl.Collator(undefined,{numeric:true,sensitivity:'base'});
...
// ★FIX: collator rápido no sort (era localeCompare caro, chamado em listas grandes)
function gdiFinalizeCachedNames(list){
  try{
    list.sort((x,y)=>
      __gdiCollator.compare(String(x.folder||''),String(y.folder||''))||
      __gdiCollator.compare(String(x.origName||x.name),String(y.origName||y.name)));
    const counts={};
...
// ★FIX: teto de 4000 itens — JSON gigante no localStorage trava a main thread (é síncrono)
function gdiPlaylistCacheSet(parent,list){
  try{
    const v=(list||[]).filter(x=>x.name&&x.rawLink).slice(0,4000).map(x=>({n:x.name,o:x.origName||x.name,f:x.folderLabel||null,
...
// ═══════════════════════════════════════════════════════════════
// FILE_VIDEO — v19.4-fix
// ★FIX 1: renderPlaylistUI com teto de 600 itens no DOM + UM listener
//         delegado (antes: 1 listener POR aula + HTML de MBs = segundos
//         de main thread por render em playlists grandes)
// ★FIX 2: durante o escaneamento multi-pasta, renderiza UMA vez (o
//         contador textual continua ao vivo); antes re-renderizava a
//         lista inteira a cada lote de pastas
// ★FIX 3: switchVideo atualiza só a classe .cur (sem re-render)
// ★FIX 4: sort com collator e ordena UMA vez no fim (era a cada lote)
// ═══════════════════════════════════════════════════════════════
...
// ★FIX 3: atualização leve do item atual (usada ao trocar de aula e durante o scan)
function markCurrentPlaylistItem(){
  const total=playlistVideos.length;
  const c=document.getElementById('gdi-pl-count')||document.getElementById('gdi-playlist-count');
  if(c)c.textContent=`${currentIndex+1} / ${total}`;
  const info=document.getElementById('gdi-nav-info');
  if(info&&total>1)info.textContent=`${currentIndex+1} / ${total}`;
  const prev=document.getElementById('gdi-btn-prev'),next=document.getElementById('gdi-btn-next');
  if(prev){prev.disabled=currentIndex<=0;prev.style.opacity=currentIndex<=0?"0.4":"1";}
  if(next){next.disabled=currentIndex>=total-1;next.style.opacity=currentIndex>=total-1?"0.4":"1";}
  const old=document.querySelector('.gdi-playlist-item.cur');
  if(old)old.classList.remove('cur');
  const el=document.querySelector(`.gdi-playlist-item[data-idx="${currentIndex}"]`);
  if(el){el.classList.add('cur');try{el.scrollIntoView({block:'nearest'})}catch(_){}}
}

// ★FIX 1: teto + delegation + classes (visual vem do CSS global)
const GDI_PL_CAP=600;
function renderPlaylistUI(){
  if(!playlistVideos.length) return;
  const total=playlistVideos.length;
  const shown=total>GDI_PL_CAP?playlistVideos.slice(0,GDI_PL_CAP):playlistVideos;
  let h="";
  shown.forEach((m,v)=>{
    ...
    h+=`<div class="gdi-playlist-item${w?' cur':''}${watched&&!w?' watched':''}" data-idx="${v}" title="${escHtml(m.name||'')}">
      <div><i class="bi bi-${w?'play-fill':watched?'check-circle-fill':'film'} me-2"></i><span style="font-weight:${w?'600':'400'};">${escHtml(m.name||'')}</span></div>
      <span class="gdi-pl-size">${watched?'\u2713 ':''}${escHtml(m.size||'')}</span>
    </div>`;
  });
  if(total>shown.length)h+=`<div style="padding:6px 12px;font-size:11px;color:#8b949e;">\u2026 +${total-shown.length} aulas (Pr\u00f3xima/Anterior e a tecla J alcan\u00e7am todas)</div>`;
  const listEl=document.getElementById("gdi-playlist-list");
  if(!listEl)return;
  listEl.innerHTML=h;
  if(!listEl.__gdiPlClick){
    listEl.__gdiPlClick=true;
    listEl.addEventListener("click",function(ev){
      const it=ev.target.closest(".gdi-playlist-item");
      if(!it)return;
      const m=parseInt(it.dataset.idx,10);
      if(!isNaN(m)&&playlistVideos[m])switchVideo(m);
    });
  }
  $("#gdi-playlist-count").text(`${currentIndex+1} / ${total}`);
  $("#gdi-playlist-wrap").show();
  const navBar=document.getElementById("gdi-player-nav");
  if(navBar&&total>1){
    navBar.style.display="flex";
    document.getElementById("gdi-nav-info").textContent=`${currentIndex+1} / ${total}`;
    ...
    document.getElementById("gdi-btn-next").disabled=currentIndex>=total-1;
    document.getElementById("gdi-btn-next").style.opacity=currentIndex>=total-1?"0.4":"1";
  }
  const cur=listEl.querySelector(`.gdi-playlist-item[data-idx="${currentIndex}"]`);
  if(cur)try{cur.scrollIntoView({block:'nearest'})}catch(_){}
}
...
function switchVideo(h){
  // ★FIX: guarda contra chamada dupla (core + extras podem delegar o mesmo clique)
  if(h<0||h>=playlistVideos.length||h===currentIndex) return;
  currentIndex=h;
  ...
  markCurrentPlaylistItem(); // ★FIX 3: era renderPlaylistUI() completo
...
// ★FIX 2/4: sort fora do scanBatch (1× no fim), teto de vídeos, collator
async function loadCrossFolderPlaylist(onProgress){
  const INITIAL_ITEMS=60,BATCH_SUBS=3,BATCH_DELAY=2000,MAX_FOLDERS=200,MAX_VIDEOS=4000;
  ...
  if(subFolders.length>MAX_FOLDERS)subFolders=subFolders.slice(0,MAX_FOLDERS);
  subFolders.sort((x,y)=>__gdiCollator.compare(x.name,y.name));
  const collected=[];let cursor=0;
  async function scanBatch(count){
    for(let k=0;k<count&&cursor<subFolders.length&&collected.length<MAX_VIDEOS;k++){
      const folder=subFolders[cursor++];
      const fpath=parentPath+encodeURIComponent(folder.name)+"/";
      const files=await gdiListAllFiles(fpath,gdiGetPw(fpath));
  ...
  if(onProgress)try{onProgress(collected.slice(),cursor,subFolders.length)}catch(_){}
  while(cursor<subFolders.length&&collected.length<MAX_VIDEOS){
...
try{
  ...
  // ★FIX 2: primeiro lote renderiza UMA vez; o resto só atualiza contador/classe
  let plRendered=false;
  const crossVideos=await loadCrossFolderPlaylist(function(partial,done,total){
    if(partial.length<2)return;
    if(!plRendered){
      plRendered=true;
      finalizeNames(partial);
      ...
      playlistVideos=dedupePlaylist(partial);
      const ci=gdiFindCurrent(playlistVideos);
      if(ci!==-1&&ci!==currentIndex){currentIndex=ci;markCurrentPlaylistItem();}
    }
...
function showResumeToast(t,onRestart){
  ...
  c.appendChild(el);
  const b=el.querySelector('gdi-toast-btn')||el.querySelector('.gdi-toast-btn');
...
/* ★FIX: playlist com classes (era estilo inline por item + 1 listener por item) */

...
console.log('[GDI core] v19.2-completo');
...
// ═══ ★ v19.2: cache de playlist multi-pasta ═══
// Ordem de leitura: playlist.json na pasta do Drive → cache local (7 dias) → escanear
function gdiPlCacheKey(p){return 'gdi-xpl::'+(window.location.host||'')+'::'+(p||'/')}
function gdiPlaylistFromJSON(arr){
  const UI=window.UI||{},out=[];
  (arr||[]).forEach(x=>{
    const link=x.l||x.link||'',name=x.n||x.name||'',folder=x.fd||x.folder||'';
    if(!link||!name)return;
    out.push({id:x.id||'',name,origName:x.o||name,folderLabel:x.f||null,
      size:formatFileSize(x.s||0),sizeBytes:Number(x.s)||0,
...
async function gdiPlaylistCacheGet(parent){
  try{
    ...
    if(r.ok){const j=await r.json();
      ...
      if(arr){const v=gdiPlaylistFromJSON(arr);
        if(v.length>1){console.log('[GDI Playlist] carregada do playlist.json do Drive ('+v.length+' aulas)');return v;}}}
  }catch(_){}
  try{
    ...
    if(raw){const o=JSON.parse(raw);
      if(o&&Array.isArray(o.v)&&o.v.length>1&&Date.now()-(o.at||0)<7*86400000){
        const v=gdiPlaylistFromJSON(o.v);
        if(v.length>1){console.log('[GDI Playlist] carregada do cache local ('+v.length+' aulas)');return v;}}}
  }catch(_){}
  return null;
}
function gdiPlaylistCacheSet(parent,list){
  try{
    const v=(list||[]).map(x=>({n:x.name,o:x.origName||x.name,f:x.folderLabel||null,
...
// ═══ ★ v19.2: fallback de PDF no core — o M17 (extras) sobrepõe com pdf.js ═══
...
// ═══════════════════════════════════════════════════════════════
// FILE_VIDEO — v19.2 COMPLETO
// • markup da playlist no template (bug: playlist invisível)
// • avanço automático: ended/complete de todos os players + vídeo nativo,
//   com guarda anti-duplo-disparo de 3s (gdiQueueNext)
// • fim de aula marca assistido (pathname, compatível com gdiMarkVideo do
//   extras quando presente) + emite watched:changed
// • window.switchVideo / playNextInPlaylist / renderPlaylistUI expostos
// • cache multi-pasta: playlist.json → localStorage → escaneamento
// ═══════════════════════════════════════════════════════════════
...
// ★ avanço automático com guarda anti-duplo-disparo
...
Bus.on('media:ready',({type,el})=>{
  ...
  el.addEventListener('ended',()=>{
    try{
      if(window.gdiMarkVideo)gdiMarkVideo();               // ★ chave dupla (M20)
      else GDIUser.markWatched(window.location.pathname);
      GDIUser.setLast(LAST_KEY());
      Bus.emit('watched:changed');
    }catch(_){}
    gdiQueueNext(); // rede de segurança p/ players sem evento ended próprio
...
// ★ expõe para os módulos (atalho J usa window.switchVideo)
...
// ★ v19.2: rawLink/sizeBytes/thumbRaw — necessários p/ o playlist.json e cache
...
try{
  ...
  // ★ v19.2: tenta playlist.json do Drive / cache local antes de escanear
  ...
  if(crossVideos.length>1){
    ...
    renderPlaylistUI();
    gdiPlaylistCacheSet(parentPath,crossVideos); // ★ salva cache local (7 dias)
  }
...
// ═══════════════════════════════════════════════════════════════
// CSS GLOBAL (★ v19.2: inclui regras de foco como segurança —
// os módulos também injetam as delas, redundância de propósito)
// ═══════════════════════════════════════════════════════════════
...
/* ★ v19.2: modo foco — redundância de segurança no core */

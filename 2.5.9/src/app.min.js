...
function file_video(i,e,t,n,a,c,l,d,o){const UI=window.UI||{},player_config=window.player_config||{};o=o||[];const s=a.includes(".m3u8")||c==="application/x-mpegURL";let r="",p="",g="";if(!UI.disable_player){if(player_config.player==="plyr"){r=`<video id="player" playsinline controls autoplay data-poster="${n}">
            ...
            </video>`,p="https://cdn.plyr.io/"+player_config.plyr_io_version+"/plyr.polyfilled.js",g="https://cdn.plyr.io/"+player_config.plyr_io_version+"/plyr.css"}else if(player_config.player==="videojs"){r=`<video id="vplayer" poster="${n}" class="video-js vjs-default-skin vjs-big-play-centered" controls autoplay preload="auto" width="100%" height="100%" data-setup='{"fluid":true,"autoplay":true}'>
              <source src="${a}" type="${s?"application/x-mpegURL":"video/mp4"}">
              <source src="${a}" type="video/webm">
              ${o.map(h=>`<track kind="subtitles" src="${escHtml(h.url)}" label="${escHtml(h.label)}" default>`).join("")}
            </video>`,p="https://vjs.zencdn.net/"+player_config.videojs_version+"/video.js",g="https://vjs.zencdn.net/"+player_config.videojs_version+"/video-js.css"}else if(player_config.player==="dplayer"){r='<div id="player-container"></div>',p="https://cdn.jsdelivr.net/npm/dplayer/dist/DPlayer.min.js",g="https://cdn.jsdelivr.net/npm/dplayer/dist/DPlayer.min.css"}else if(player_config.player==="jwplayer"){r='<div id="player"></div>',p="https://content.jwplatform.com/libraries/IDzF9Zmk.js"}}const f=`<div class="gdi-player-wrap" style="width:100%;margin:0 auto;">${r}</div>
<div id="gdi-playlist-wrap" style="margin-top:16px;display:none;">
  ...
  <div id="gdi-playlist-list" style="max-height:240px;overflow-y:auto;border:1px solid rgba(255,255,255,0.12);border-radius:8px;padding:6px;background:rgba(0,0,0,0.2);"></div>
</div>`,u=UI.disable_video_download?"":renderDownloadButtons(a,e,{showMedia:!0});$("#content").html(_viewerCard('<i class="bi bi-camera-video-fill gdi-icon-video"></i>',i,t,f,u)),$(".gdi-viewer").css({"max-width":"1200px","width":"96%","margin":"0 auto"});if(g){const h=document.createElement("link");h.rel="stylesheet",h.href=g,document.head.appendChild(h)}window._gdiPlayerInstance=null;function initPlayerEvents(){if(player_config.player==="plyr"){const m=new Plyr("#player",{autoplay:!0,keyboard:{focused:!0,global:!0}});window._gdiPlayerInstance=m,m.on("ready",()=>m.play().catch(()=>{})),m.on("ended",playNextInPlaylist)}else if(player_config.player==="videojs"){const m=videojs("vplayer",{autoplay:!0,playbackRates:[.5,.75,1,1.25,1.5,2],controlBar:{pictureInPictureToggle:!0}});window._gdiPlayerInstance=m,m.ready(function(){this.play().catch(()=>{}),this.el().addEventListener("keydown",function(v){v.target.tagName!=="INPUT"&&(v.key===" "?(v.preventDefault(),m.paused()?m.play():m.pause()):v.key==="f"?m.isFullscreen()?m.exitFullscreen():m.requestFullscreen():v.key==="m"?m.muted(!m.muted()):v.key==="ArrowRight"?m.currentTime(m.currentTime()+10):v.key==="ArrowLeft"?m.currentTime(Math.max(0,m.currentTime()-10)):v.key==="ArrowUp"?m.volume(Math.min(1,m.volume()+.1)):v.key==="ArrowDown"&&m.volume(Math.max(0,m.volume()-.1)))})}),m.on("ended",playNextInPlaylist)}else if(player_config.player==="dplayer"){const m=new DPlayer({container:document.getElementById("player-container"),autoplay:!0,screenshot:!0,video:{url:a,pic:n,type:s?"hls":"auto"},subtitle:o.length?{url:o[0].url,type:"webvtt"}:void 0});window._gdiPlayerInstance=m,m.play(),m.on("ended",playNextInPlaylist)}else if(player_config.player==="jwplayer"){const m=jwplayer("player").setup({file:a,type:c,autostart:!0,image:n,width:"100%",aspectratio:"16:9",title:i,description:"Powered by Google Drive Index",tracks:o.map(v=>({file:v.url,kind:"captions",label:v.label,default:!0})),captions:{color:"#f3f378",fontSize:14,backgroundOpacity:50,edgeStyle:"raised"}});window._gdiPlayerInstance=m,m.on("complete",playNextInPlaylist)}}if(p){if(window.Plyr||window.videojs||window.DPlayer||window.jwplayer)initPlayerEvents();else{const h=document.createElement("script");h.src=p,h.onload=initPlayerEvents,document.head.appendChild(h)}}let playlistVideos=[],currentIndex=-1;function renderPlaylistUI(){if(!playlistVideos.length)return;let h="";playlistVideos.forEach((m,v)=>{const w=v===currentIndex,y=w?"background:var(--bs-primary,#1f6feb);color:#fff;":"background:rgba(255,255,255,0.05);color:var(--gdi-text,#e6edf3);";h+=`<div class="gdi-playlist-item" data-idx="${v}" style="padding:8px 12px;margin:3px 0;border-radius:6px;cursor:pointer;display:flex;align-items:center;justify-content:space-between;font-size:13px;transition:background 0.2s;${y}">
    ...
    </div>`}),$("#gdi-playlist-list").html(h),$("#gdi-playlist-count").text(`${currentIndex+1} / ${playlistVideos.length}`),$("#gdi-playlist-wrap").show(),$(".gdi-playlist-item").off("click").on("click",function(){const m=parseInt($(this).data("idx"));!isNaN(m)&&playlistVideos[m]&&switchVideo(m)})}function switchVideo(h){if(h<0||h>=playlistVideos.length)return;currentIndex=h;const m=playlistVideos[currentIndex];m.pageUrl&&history.pushState(null,m.name,m.pageUrl),$(".gdi-file-header-name").text(m.name),$(".gdi-file-header-meta").text(m.size);const v=window.drive_names?window.drive_names[window.current_drive_order||0]:"Drive";if($("title").html(`${v} - ${m.name}`),UI.disable_video_download||$(".gdi-viewer-footer").html(renderDownloadButtons(m.streamUrl,encodeURIComponent(m.name),{showMedia:!0})),player_config.player==="plyr"&&window._gdiPlayerInstance)window._gdiPlayerInstance.source={type:"video",title:m.name,sources:[{src:m.streamUrl,type:m.streamUrl.includes(".m3u8")||m.mimeType==="application/x-mpegURL"?"application/x-mpegURL":"video/mp4"}]},window._gdiPlayerInstance.play().catch(()=>{});else if(player_config.player==="videojs"&&window._gdiPlayerInstance)window._gdiPlayerInstance.src({src:m.streamUrl,type:m.streamUrl.includes(".m3u8")||m.mimeType==="application/x-mpegURL"?"application/x-mpegURL":"video/mp4"}),window._gdiPlayerInstance.play().catch(()=>{});else if(player_config.player==="dplayer"&&window._gdiPlayerInstance)window._gdiPlayerInstance.switchVideo({url:m.streamUrl,pic:m.poster,type:m.streamUrl.includes(".m3u8")||m.mimeType==="application/x-mpegURL"?"hls":"auto"}),window._gdiPlayerInstance.play();else if(player_config.player==="jwplayer"&&window._gdiPlayerInstance)window._gdiPlayerInstance.load([{file:m.streamUrl,type:m.mimeType,image:m.poster,title:m.name}]),window._gdiPlayerInstance.play();else{const w=document.getElementById("player")||document.getElementById("vplayer");w&&(w.src=m.streamUrl,w.play().catch(()=>{}))}renderPlaylistUI()}function playNextInPlaylist(){currentIndex>=0&&currentIndex<playlistVideos.length-1&&switchVideo(currentIndex+1)}const folderPath=window.location.pathname.split("/").slice(0,-1).join("/")+"/";requestListPath(folderPath,{},function(h){if(!h||!h.data||!h.data.files)return;const m=h.data.files;playlistVideos=[],m.forEach(v=>{if(FILE_TYPES.video.includes(v.fileExtension)||v.mimeType&&v.mimeType.includes("video")){const w=encodeURIComponent(v.name),y=UI.second_domain_for_dl?UI.downloaddomain+v.link:window.location.origin+v.link,_=folderPath+w+"?a=view",b=v.thumbnailLink?v.thumbnailLink.replace("s220","s0"):UI.poster;playlistVideos.push({id:v.id,name:v.name,size:formatFileSize(v.size),streamUrl:y,pageUrl:_,mimeType:v.mimeType,poster:b})}}),currentIndex=playlistVideos.findIndex(v=>v.name===i||v.streamUrl===a),currentIndex===-1&&playlistVideos.length>0&&(playlistVideos.unshift({name:i,size:t,streamUrl:a,pageUrl:window.location.href,mimeType:c,poster:n}),currentIndex=0),renderPlaylistUI()},null)}

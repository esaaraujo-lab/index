...
console.log('[GDI app] build v18.0-cycle23');
...
/* ── v18: autoplay overlay + pular intro + progresso por módulo ── */
...
#gdi-next-ep{position:fixed;right:20px;bottom:90px;z-index:9500;background:rgba(13,15,20,.97);
  border:1px solid rgba(255,255,255,.18);border-radius:14px;padding:14px 16px;max-width:340px;
  box-shadow:0 14px 44px rgba(0,0,0,.6);display:none;}
#gdi-next-ep.show{display:block;animation:gdi-card-in .25s ease;}
...
// C3: progresso por módulo — calculado em 2º plano (sequencial, cache 45s)
...
function appendUnifiedRows(mode,items,basePath){
  ...
  if(!isSearch&&mode==='path')setTimeout(gdiModuleProgress,0); // C3
}
...
// chave = pasta completa do vídeo (ex.: /6:/ALFACON/CURSO-A/)
...
Bus.on('media:ready',({type,el})=>{
  ...
  const upd=()=>{
    try{
      ...
      let show=false;
      if(S&&S>0)show=tm>0.4&&tm<S-0.3&&tm<180;      // aprendida: aparece só antes do ponto
      else show=tm>1&&tm<120;                        // não aprendida: janela de aprendizado
      skipBtn.innerHTML=S
...
// ═══ N1: AUTOPLAY COM CONTAGEM (estilo Netflix) ═══
function gdiQueueNext(){
  try{
    if(currentIndex<0||currentIndex>=playlistVideos.length-1){return;}
    const nxt=playlistVideos[currentIndex+1];
    let ov=document.getElementById('gdi-next-ep');
    if(ov)ov.remove();
    ov=document.createElement('div');ov.id='gdi-next-ep';
    document.body.appendChild(ov);
    let left=8,timer=null;
    const close=()=>{clearInterval(timer);ov.classList.remove('show');};
    const go=()=>{close();playNextInPlaylist();};
    const render=()=>{ov.innerHTML=`
      <div style="font-size:11px;color:#8b949e;text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px;">A seguir</div>
      <div style="font-weight:600;color:#f0f6fc;margin-bottom:10px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:300px;">${escHtml(nxt.name)}</div>
      <div style="display:flex;gap:8px;align-items:center;">
        <button id="gdi-nep-go" class="gdi-btn gdi-btn-primary"><i class="bi bi-play-fill"></i> Assistir (<span class="nep-count">${left}</span>)</button>
        <button id="gdi-nep-cancel" class="gdi-mode-btn" title="Cancelar autoplay">\u2715</button>
      </div>`;
      document.getElementById('gdi-nep-go').addEventListener('click',go);
      document.getElementById('gdi-nep-cancel').addEventListener('click',()=>{close();showToast('Autoplay cancelado');});
    };
    render();ov.classList.add('show');
    timer=setInterval(()=>{
      left--;
      if(left<=0){go();return;}
      const c=ov.querySelector('.nep-count');if(c)c.textContent=left;
    },1000);
  }catch(_){}
}
...
// 'ended' dos players agora passa pelo overlay de "a seguir"
...
// ═══ ESTADO DO USUÁRIO v4 — + intro (pular intro por curso) + srs (revisão espaçada) ═══
const GDIUser=(()=>{
  const SCHEMA=4;
...
// EXTRAS v18
// ═══════════════════════════════════════════════════════════════
console.log('[GDI Extras] v18.0 ativo');
...
wrap.innerHTML=<button id="gdi-notes-export" class="gdi-mode-btn" style="padding:3px 10px;font-size:11px;" title="Baixar anota\u00e7\u00f5es"><i class="bi bi-download"></i> Exportar</button> <button id="gdi-review-btn" class="gdi-mode-btn" style="padding:3px 10px;font-size:11px;" title="Tocar s\u00f3 os trechos anotados (${GDI_REVIEW_SPAN}s cada)"><i class="bi bi-fast-forward-fill"></i> Revis\u00e3o</button>;
...
if(list&&!list.__gdiNObs){list._gdiNObs=true;
...
m.innerHTML=<button class="gdi-mode-btn" id="gdi-exp-md" style="justify-content:flex-start;font-size:12px;"><i class="bi bi-markdown"></i> Markdown (.md)</button> <button class="gdi-mode-btn" id="gdi-exp-anki" style="justify-content:flex-start;font-size:12px;"><i class="bi bi-collection"></i> Anki / texto (.txt)</button>;
...
let name='aula';try{name=decodeURIComponent(window.location.pathname.split('/').pop()||'aula')}catch(){}
...
try{return '/fallback::'+(new URLSearchParams(p.split('?')[1]||'').get('id')||'')}catch(){return ''}
}
return p.split('?')[0];
}
...
const name=(()=>{try{return decodeURIComponent(seg.pop()||'')}catch(){return seg.pop()||''}})();
const folder=(()=>{try{return seg.length?decodeURIComponent(seg[seg.length-1]):''}catch(_){return ''}})();
return{name:name.replace(/.[a-z0-9]+
/i,''),folder};
}
...
hours/=3600;
const due=gdiSrsDueList().length; // D4
...
if(target){
html+=<div style="display:flex;align-items:center;gap:12px;min-width:0;flex:1;"> <i class="bi bi-play-circle-fill" style="font-size:30px;color:#7aa2ff;"></i> <div style="min-width:0;"> <div style="font-size:11px;color:#8b949e;text-transform:uppercase;letter-spacing:.06em;">Continuar em ${escHtml(gdiDriveLabel())}</div> <div style="font-weight:600;color:#f0f6fc;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escHtml(lbl.name)}</div> <div style="font-size:12px;color:#8b949e;">${escHtml(lbl.folder)}${r?' \u00b7 parou em '+gdiFmtTime(r.t):''}</div> </div></div> ${resumeBtn};
}
if(isHome){
html+=<div style="display:flex;gap:16px;font-size:12px;color:#8b949e;flex-wrap:wrap;"> ${streak>0?<span title="Dias seguidos com atividade de estudo"><i class="bi bi-fire" style="color:#ff922b;"></i> 
streakdia
{streak>1?'s':''} seguidos</span>:''} ${hours>0?<span title="Soma das posi\u00e7\u00f5es salvas (estimativa)"><i class="bi bi-clock-history"></i> \u2248 
{String(hours.toFixed(1)).replace('.',',')}h assistidas</span>`:''}
</div>`;
if(due>0){
html+=`<div style="flex-basis:100%;margin-top:2px;">
<button id="gdi-srs-open" class="gdi-mode-btn" style="font-size:12px;" title="Revis\u00e3o espa\u00e7ada das suas anota\u00e7\u00f5es (1, 7 e 30 dias)"><i class="bi bi-mortarboard-fill" style="color:#ffd43b;"></i> Revisar
{due} anota\u00e7\u00e3${due>1?'\u00f5es':'o'} de hoje</button>
</div>; } if(hist.length){ html+=<div style="flex-basis:100%;display:flex;gap:6px;flex-wrap:wrap;align-items:center;margin-top:2px;">
<span style="font-size:11px;color:#8b949e;"><i class="bi bi-clock-history"></i> Recentes:</span>
{hist.slice(0,6).map(h=>`<a class="gdi-mode-btn" data-gdi-go style="padding:2px 8px;font-size:11px;" href="
{escHtml(gdiPlayerHref(h.path))}" title="
escHtml(h.name∣∣ 
′′
 )">
{escHtml((h.name||'').slice(0,26)||'Aula')}</a>).join('')} </div>;
}
}else if(streak>0){
...
const due=e?e.due:(n.at+86400000); // 1 dia após criar
...
return decodeURIComponent(seg.pop()||'').replace(/.[a-z0-9]+
/i,'')||'Aula';
}catch(_){return 'Aula'}
}
function gdiSrsOpen(){
...
<span style="font-size:11px;color:#8b949e;text-transform:uppercase;letter-spacing:.06em;">\ud83e\uddd0 Revis\u00e3o
{idx+1} de 
{due.length}</span>
<button class="gdi-mode-btn" id="gdi-srs-close" style="padding:2px 8px;font-size:11px;">\u2715</button>
</div>
<div style="font-size:12px;color:#7aa2ff;margin-bottom:4px;">
{escHtml(gdiSrsLessonLabel(n.key))}
n.t!=null? 
′
...
′
 +gdiFmtTime(n.t): 
′′
 </div><divstyle="font−size:15px;line−height:1.5;margin−bottom:16px;">
{escHtml(n.text)}</div>
...
if(countEl.__gdiProg)return;countEl._gdiProg=true;
...
total++;
let w=false;try{w=GDIUser.isWatched(href.split('?')[0])}catch(){}
...
let html='';
if(total){
const pct=Math.round(done/total100);
html+=<i class="bi bi-bar-chart-fill" style="color:#7aa2ff;"></i> <span>${done}/${total} assistido${done===1?'':'s'} (${pct}%)</span> <div style="flex:1;max-width:160px;height:5px;background:rgba(255,255,255,.1);border-radius:3px;overflow:hidden;"> <div style="height:5px;width:${pct}%;background:${pct>=100?'#1a7f37':'#1f6feb'};transition:width .4s;"></div> </div>;
...
if(!FILE_TYPES.video.includes((f.fileExtension||'').toLowerCase()))continue;
if(/.part-/i.test(f.name))continue;
const bytes=Number(f.size)||0;if(bytes>0&&bytes<10241024)continue;
total++;
try{if(GDIUser.isWatched(base+encodeURIComponent(f.name)))done++;}catch(){}
}
}
...
}catch(){showToast('N\u00e3o foi poss\u00edvel calcular o progresso do curso');}

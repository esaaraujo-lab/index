...
// ─────────────────────────────────────────────────────────────
// Lista TODAS as páginas de uma pasta (Drive API: máx 100/página).
// Timeout de 20s por página — nunca trava. Modo quiet não pisca UI.
// ─────────────────────────────────────────────────────────────
async function gdiListAllFiles(path,pw){
  let out=[],token='',idx=0;
  const withTimeout=(p,ms)=>Promise.race([p,new Promise(r=>setTimeout(()=>r(null),ms))]);
  for(let guard=0;guard<40;guard++){
    const page=await withTimeout(new Promise(res=>{
      try{
        requestListPath(path,{password:pw||'',page_token:token,page_index:idx},function(d){
          res({files:(d&&d.data&&Array.isArray(d.data.files))?d.data.files:[],next:(d&&d.nextPageToken)?d.nextPageToken:''});
        },null,1,true);
      }catch(_){res(null)}
    }),20000);
    if(!page||!Array.isArray(page.files))break;
...
// q=true = modo quiet (varreduras em 2º plano não escrevem na UI)
function requestListPath(i,e,t,n,a=3,c=!1,q=!1){const l={id:e.id||"",type:"folder",password:e.password||"",page_token:e.page_token||"",page_index:e.page_index||0},d=a!=null&&a>=0?a:3;q||($("#update").show(),$("#update").html('<div class="gdi-alert gdi-alert-info">Connecting\u2026</div>')),c&&(i="/0:fallback");async function o(s){try{const r=await fetch(c?"/0:fallback":i,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(l)});if(!r.ok)throw new Error("Request failed with status "+r.status);const p=await r.json();p&&p.error&&p.error.code===401?($("#update").hide(),askPassword(i)):p&&p.data===null?(document.getElementById("spinner")?.remove(),$("#list").html(`<div class="gdi-empty"><i class="bi bi-exclamation-circle"></i><p>Server didn't send any data.</p></div>`),$("#update").hide()):p&&p.data&&(t(p,i,l),$("#update").hide())}catch(r){s>0?(q||$("#update").html(`<div class="gdi-alert gdi-alert-info">Retrying\u2026 (${s} left)</div>`),await sleep(2e3),await o(s-1)):(q||($("#update").html('<div class="gdi-alert gdi-alert-error">Unable to connect. Please try again.</div>'),$("#list").html(`<div class="gdi-empty"><i class="bi bi-wifi-off"></i><p>${escHtml(String(r))}</p></div>`)),$("#update").hide())}}o(d)}
...
function renderPlaylistUI(){
  ...
  // Rola a playlist até o item atual (útil com 100+ itens)
...
// ─────────────────────────────────────────────────────────────
// PLAYLIST ANINHADA — inclui TODOS os vídeos de TODAS as subpastas
// irmãs (antes pegava só o 1º vídeo de cada). Varredura sequencial
// (sem corrida de contador) e timeout por página (nunca trava).
// ─────────────────────────────────────────────────────────────
async function loadCrossFolderPlaylist(){
  ...
  if(!subFolders.length){console.log('[GDI Playlist] aninhada: nenhuma subpasta em',parentPath);return [];}
  ...
  console.log('[GDI Playlist] aninhada: varrendo',subFolders.length,'subpastas de',parentPath);
  const collected=[];
  ...
  // Ordena: pasta (numérico) → nome (numérico). "Aula 02 - Parte 10" vem depois da Parte 09
  collected.sort((x,y)=>
    String(x.folder||'').localeCompare(String(y.folder||''),undefined,{numeric:true,sensitivity:'base'})||
    String(x.name).localeCompare(String(y.name),undefined,{numeric:true,sensitivity:'base'}));
  // Renomeia APENAS duplicatas com o nome da pasta (preserva "Parte 01" dos únicos)
  const counts={};
  collected.forEach(v=>{counts[v.name]=(counts[v.name]||0)+1});
  collected.forEach(v=>{if(counts[v.name]>1&&v.folderLabel)v.name=v.folderLabel+' - '+v.name});
  console.log('[GDI Playlist] aninhada: total',collected.length,'vídeos');
...
(async function buildPlaylist(){
  ...
  let videos=buildPlaylistFromFiles(allFiles,folderPath);
  console.log('[GDI Playlist] pasta atual:',videos.length,'vídeos de',allFiles.length,'itens');

  if(videos.length<=1){
    const crossVideos=await loadCrossFolderPlaylist();
    if(crossVideos.length>1){
      playlistVideos=crossVideos;
      const gdiNormPath=p=>{try{return decodeURIComponent(String(p||'')).replace(/\/+$/,'')}catch(_){return String(p||'').replace(/\/+$/,'')}};
      currentIndex=playlistVideos.findIndex(v=>(gdiNormPath(v.folder)===gdiNormPath(folderPath)&&v.name===i)||v.streamUrl===a||v.name===i);
      console.log('[GDI Playlist] aninhada:',playlistVideos.length,'itens, currentIndex=',currentIndex);
      if(currentIndex===-1){
  ...
  playlistVideos=dedupePlaylist(videos);
  currentIndex=playlistVideos.findIndex(v=>v.name===i||v.streamUrl===a);
  console.log('[GDI Playlist] currentIndex=',currentIndex);
...
// ── Resume: continuar de onde parou (funciona ao trocar de vídeo na playlist) ──
...
function file_audio(i,e,t,n,a,c,l){const UI=window.UI||{};l=l||[{name:i,url:n,cover:UI.audioposter}];const d=UI.disable_player?"":'<div id="aplayer-container" style="max-width:680px;margin:0 auto;"></div>',o=UI.disable_audio_download?"":renderDownloadButtons(n,e,{showMedia:!0});if($("#content").html(_viewerCard('<i class="bi bi-music-note-beamed gdi-icon-audio"></i>',i,t,d,o)),UI.disable_player)return;const s=document.createElement("link");s.rel="stylesheet",s.href="https://cdn.jsdelivr.net/npm/aplayer@1.10.1/dist/APlayer.min.css",document.head.appendChild(s);const r=document.createElement("script");if(r.src="https://cdn.jsdelivr.net/npm/aplayer@1.10.1/dist/APlayer.min.js",r.onload=function(){window._gdiAPlayer=new APlayer({container:document.getElementById("aplayer-container"),mini:!1,autoplay:!1,theme:"#4d9fec",loop:"all",order:"list",preload:"auto",volume:.7,listFolded:!1,audio:l})},document.head.appendChild(r),l.length<=1){(async()=>{const p=window.location.pathname.split("/").slice(0,-1).join("/")+"/";const apFiles=await gdiListAllFiles(p,localStorage.getItem('password'+p)||'');if(!apFiles.length)return;const f2=apFiles.filter(u=>FILE_TYPES.audio.includes(u.fileExtension));if(f2.length>1){const u=f2.map(m=>({name:m.name,url:UI.second_domain_for_dl?UI.downloaddomain+m.link:window.location.origin+m.link,cover:UI.audioposter}));window._gdiAPlayer&&window._gdiAPlayer.destroy(),window._gdiAPlayer=new APlayer({container:document.getElementById("aplayer-container"),mini:!1,loop:"all",order:"list",preload:"auto",volume:.7,audio:u});const h=u.findIndex(m=>m.url===n);h>0&&window._gdiAPlayer.list.switch(h)}})()}
  ...
  // ── Sleep mode: automático após 8s tocando + botão manual 🌙 ──
  initSleepMode('audio');}
...
// ============================================================
// RESUME — "Continuar de onde parou" (por navegador do usuário)
// ============================================================
const GDIResume=(()=>{
...
// Polling de segurança (tenta o seek por até 6s) + logs no Console
function attachResumeTracking(media,getKey){
  ...
  const save=()=>{const k=key();if(!k)return;const before=GDIResume.get(k);GDIResume.set(k,media.currentTime,dur());const after=GDIResume.get(k);if(!before&&after)console.log('[GDI Resume] primeira posição salva:',gdiFmtTime(after.t),k)};

  function applySeek(reason){
    const k=key(); if(!k||appliedKey===k)return;
    const sv=GDIResume.get(k);
    if(!sv){
    ...
    const d=dur();
    if(d&&sv.t>d-15){GDIResume.del(k);appliedKey=k;console.log('[GDI Resume] vídeo quase concluído, ignorando');return}
    try{
      media.currentTime=sv.t;
      appliedKey=k;
      console.log('[GDI Resume] retomando em',gdiFmtTime(sv.t),'via',reason,'→',k);
      showResumeToast(sv.t,()=>{try{media.currentTime=0}catch(e){}GDIResume.del(k)});
    }catch(e){console.warn('[GDI Resume] falha no seek',e)}
  }

  media.addEventListener('timeupdate',()=>{const n=Date.now();if(n-lastSave>5000){lastSave=n;save()}});
  media.addEventListener('pause',save);
  media.addEventListener('ended',()=>{GDIResume.del(key());console.log('[GDI Resume] concluído:',key())});

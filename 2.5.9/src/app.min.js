...
console.log('[GDI app] build v16.0-auth-continue');

// ═══════════════════════════════════════════════════════════════
// v16.0 — 1) Botões Entrar/Sair (/login, /logout) com estado real
//         2) Progresso 100% vinculado à conta logada (já existente,
//            agora com flush no logout)
//         3) Card "Continuar": aparece na PASTA DO CURSO quando a
//            aula pausada pertence àquela subárvore; auto-recupera
//            o alvo a partir de watched/resume se "last" faltar
// ═══════════════════════════════════════════════════════════════
...
// (revisão 3x: chave isIos duplicada da v15.1 removida)
...
// Endereço navegável = começa com "/" e não é um "/fallback" pelado
...
// ★ Item 1: slot de autenticação (Entrar/Sair) na navbar
...
// endereço completo (com ?id=...&a=view) para "última aula"/histórico
...
// ═══════════════════════════════════════════════════════════════
// ESTADO DO USUÁRIO (conta logada) — progresso 100% na conta
// ═══════════════════════════════════════════════════════════════
...
const GDIUser=(()=>{
  ...
  async function _load(){
    ...
    try{
      ...
      // heurística de sessão: resposta ok e NÃO redirecionada ao /login
...
// ═══════════════════════════════════════════════════════════════
// ITEM 1: botões Entrar/Sair na navbar (/login e /logout)
// ═══════════════════════════════════════════════════════════════
...
// EXTRAS
// ═══════════════════════════════════════════════════════════════
console.log('[GDI Extras] v16.0 ativo');
...
// ═══════════════════════════════════════════════════════════════
// ITEM 3: CARD "CONTINUAR" — contexto de pasta + auto-recuperação
// • Home: card completo (Continuar + streak + horas + recentes)
// • Pasta do curso: card compacto SOMENTE se a aula pausada
//   pertence à subárvore da pasta aberta (UX contextual)
// • Se "last" faltar/corrompido, o alvo é reconstruído a partir do
//   registro mais recente de watched/resume (auto-cura)
// ═══════════════════════════════════════════════════════════════
...
function gdiContinueCard(){
  ...
  if(!isHome){
    // contexto: a pasta só exibe o card se a aula parada é dela
    if(!target)return;
  ...
  if(target){
    html+=`<div style="display:flex;align-items:center;gap:12px;min-width:0;flex:1;">
      ...
      <a class="gdi-btn gdi-btn-primary" href="${escHtml(target)}"><i class="bi bi-play-fill"></i> Retomar</a>`;
  }
  if(isHome){

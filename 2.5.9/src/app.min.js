...
console.log('[GDI app] build v13.0-unify');

// ═══════════════════════════════════════════════════════════════
// v13.0 — PROBLEMA 3 (parte 1): TODO o CSS agora vive em UM bloco,
// injetado uma única vez. Antes: 5 pedaços espalhados, e o CSS do
// modo estudo era reinjetado a cada página de vídeo (vazamento).
// ═══════════════════════════════════════════════════════════════
(function(){if(document.getElementById('gdi-style'))return;const s=document.createElement('style');s.id='gdi-style';s.textContent=`
/* ── correção de largura (URL de download não estoura o vídeo) ── */
...
/* ── modo estudo (dividido / foco aula / foco material) ── */
...
// ═══════════════════════════════════════════════════════════════
// v13.0 — PROBLEMA 2 (parte 1): Barramento de eventos (Bus).
// As partes do site passam a SE AVISAR em vez de ficarem
// "espionando" com timers. Escopo 'page' = limpo a cada navegação;
// escopo 'global' = sobrevive às navegações (ex.: Pomodoro).
// ═══════════════════════════════════════════════════════════════
const Bus=(()=>{
  const m=new Map();
  function add(e,f,scope){if(!m.has(e))m.set(e,[]);m.get(e).push({f,scope})}
  return{
    on(e,f){add(e,f,'page');return()=>Bus.off(e,f)},
    onGlobal(e,f){add(e,f,'global')},
    off(e,f){const a=m.get(e);if(!a)return;const i=a.findIndex(x=>x.f===f);if(i>=0)a.splice(i,1)},
...
// Anuncia um elemento de mídia UMA única vez (vídeo/áudio prontos).
...
// ═══════════════════════════════════════════════════════════════
// v13.0 — PROBLEMA 3 (parte 2): as TRÊS funções de lista (pasta,
// fallback e busca) eram ~90% idênticas. Agora existe UMA só
// (appendUnifiedRows) e as três antigas viram apelidos de uma
// linha — nada nos demais pontos do código precisou mudar.
// ═══════════════════════════════════════════════════════════════
function appendUnifiedRows(mode,items,basePath){
  ...
  <span class="gdi-row-acts"></span>
    ...
    // README/HEAD.md (apenas modos pasta/fallback, na última página — comportamento original)
...
// Avisa o site de que existe um vídeo (uma única vez por elemento).
...
<div class="gdi-study" id="gdi-study">
 ...
 $("#content").html(f);
gdiAnnounceVideoFromDOM(); // plyr/videojs já têm <video> no HTML neste ponto

// ── Modos de layout + zoom no FM (CSS agora é global) ──
...
function switchVideo(h){
  ...
  try{gdiBuildMaterials();}catch(_){}                    // v13.1: refaz os materiais da nova aula
}
...
// ─────────────────────────────────────────────────────────────
// MATERIAIS — v13.1: reconstruídos a cada troca de aula.
// Antes a busca rodava UMA vez (na 1ª aula aberta) e o painel
// ficava congelado ao navegar pela playlist. Agora:
//  • os caminhos são recalculados da URL atual a cada busca;
//  • cada busca tem um "nº de geração": se a aula mudar durante
//    a busca, o resultado antigo é descartado (sem mistura);
//  • iframes já carregados são reaproveitados (voltar à aula
//    anterior é instantâneo).
// ─────────────────────────────────────────────────────────────
...
async function gdiFindPdfs(fPath,pPath){
  ...
  const pw0=localStorage.getItem('password'+fPath)||'';
  ...
  if(!found.length){
    const pw1=localStorage.getItem('password'+pPath)||'';
...
let _gdiMatGen=0;             // controle anti-mistura entre aulas
async function gdiBuildMaterials(){
  ...
  try{
    ...
    if(gen!==_gdiMatGen)return; // a aula mudou durante a busca → descarta
    if(!pdfs.length){
      tabsEl.innerHTML='';
...
gdiBuildMaterials(); // primeira construção (ao abrir a aula)
// ─────────────────────────────────────────────────────────────
...
// ═══════════════════════════════════════════════════════════════
// v13.0 — PROBLEMA 2 (parte 2): o trecho abaixo ANTIGAMENTE ficava
// "espionando" a cada 250ms procurando o vídeo. Agora o vídeo
// AVISA quando fica pronto (evento 'media:ready') e só então
// conectamos retomada, cronômetro das anotações e "assistido".
// ═══════════════════════════════════════════════════════════════
...
if(l.length<=1){
  (async()=>{
    ...
    const apFiles=await gdiListAllFiles(p,localStorage.getItem('password'+p)||'');
...
// v13: retomada de áudio via evento (era um timer de 250ms antes)
...
// ═══════════════════════════════════════════════════════════════
// ESTADO DO USUÁRIO — v13.0: PROBLEMA 1 CORRIGIDO
// • Um único caderno: o do servidor (GDIUser). O antigo caderno do
//   navegador (GDIResume/localStorage 'gdi-resume-v2') foi APOSENTADO.
// • Migração automática: na primeira visita após esta atualização,
//   tudo que estava no navegador é copiado para a conta (o registro
//   mais recente vence) e o caderno antigo é apagado.
// • Etiqueta de versão (v:2): se no futuro o formato mudar de novo,
//   o site sabe converter os dados antigos sem perder nada.
// ═══════════════════════════════════════════════════════════════
...
const GDIUser=(()=>{
  ...
  // Converte formatos antigos para o atual (cadeia de migrações)
  function migrate(s){
  ...
  // Mudança única: localStorage → conta (executa UMA vez por navegador)
  ...
  // Não deixar a lista de retomadas crescer para sempre
  function prune(st){
...
// ═══════════════════════════════════════════════════════════════
// RETOMADA — v13.0: agora consulta SOMENTE a conta (GDIUser) e
// não tem mais o timer de "tentativa a cada 300ms". Ela roda
// quando o vídeo avisa que está pronto e/ou quando a conta carrega.
// ═══════════════════════════════════════════════════════════════
function attachResumeTracking(media,getKey){
  if(!media||media.__gdiResume)return;
  media.__gdiResume=true;
  let lastSave=0,appliedKey=null,settled=false,loggedEmpty=false;
  ...
  function applySeek(reason){
    if(settled||appliedKey!==null&&appliedKey===key())return;
    if(!GDIUser.loaded())return;               // espera o caderno da conta carregar
    ...
    if(!sv){
      ...
      settled=true;                            // não tenta de novo sem necessidade
      return;
    }
    if(sv.t<GDIUser.MIN_SAVE)return;
    if(media.readyState<1)return;              // vídeo ainda não está pronto
    const d=dur();
    if(d&&sv.t>d-15){GDIUser.delResume(k);appliedKey=k;return}
    try{
  ...
  media.addEventListener('timeupdate',()=>{
    if(!appliedKey)applySeek('timeupdate');    // rede de segurança orientada a evento
  ...
  Bus.on('user:ready',()=>applySeek('user-ready'));
...
// ═══════════════════════════════════════════════════════════════
// MODO DESCANSO — v13.0: sem timers de procura. Ele é avisado
// quando o vídeo/áudio fica pronto e conecta os eventos.
// ═══════════════════════════════════════════════════════════════
function initSleepMode(type){
  ...
  if(type==='audio'){
    // Conecta quando o player de áudio avisar que está pronto
    ...
    // Áudio: sai do descanso com qualquer interação
    ...
    // Vídeo: quando a aula termina, desliga o descanso
...
// ═══════════════════════════════════════════════════════════════
// POMODORO v2.3 — CSS migrado para o bloco global (Problema 3) e
// a checagem "tem mídia na página?" agora é por AVISO (Bus), sem
// o timer de 1,5s olhando a todo momento.
// ═══════════════════════════════════════════════════════════════
;(function(){
  function gdiPomBoot(){
    ...
    // v13: visibilidade do 🍅 por eventos (sem setInterval)
    ...
    updateUI();
    console.log('[GDI Pomodoro] v2.3 pronto (eventos, sem espionagem)');
  }
...
// ═══════════════════════════════════════════════════════════════
// v14 — NÍVEIS 1 a 4
// 1: velocidade salva · atalhos · marcas na timeline · filtro playlist
// 2: continuar de onde parou · progresso da pasta · dias seguidos/horas
// 3: exportar anotações (Markdown/Anki) · modo revisão
// 4: senhas protegidas (usa gdiGetPw/gdiSetPw) · cache com limite
// ═══════════════════════════════════════════════════════════════
console.log('[GDI Extras] v14 ativo (níveis 1–4)');

// ── CSS próprio (injetado uma única vez) ──
...
// ── Helpers ──
function gdiWhenReady(selector,cb,tries){tries=tries==null?15:tries;
...
// ── NÍVEL 4a: senhas ofuscadas + migração automática das antigas ──
...
(function(){ // move senhas antigas (texto puro) para o formato protegido, uma vez
  try{
...
// ── NÍVEL 1a: velocidade do vídeo salva (vale para todas as aulas) ──
(function(){
  const RKEY='gdi-rate';let applying=false;
  const getR=()=>{const v=parseFloat(localStorage.getItem(RKEY));return(v>=0.25&&v<=4)?v:null};
  document.addEventListener('ratechange',e=>{ // usuário mudou a velocidade → guarda
...
// ── NÍVEL 1b + 3 (atalhos): N próxima aula · P anterior · [ ] pulam anotações ──
...
const GDI_REVIEW_SPAN=20; // segundos reproduzidos por trecho no modo revisão
let gdiReviewWarned=false;
...
// ── MONTAGEM por página (roda a cada navegação) ──
...
// ── NÍVEL 3a: botões Exportar + Revisão no painel de anotações ──
function gdiSetupNotesButtons(head){
  ...
  document.getElementById('gdi-review-btn').addEventListener('click',()=>{
    ...
    if(gdiReviewOn){
      ...
      if(v&&notes.length){ // começa pela primeira anotação à frente
...
// ── NÍVEL 2a + 2c: card "Continuar de onde parou" + dias seguidos/horas ──
let gdiHomeCardKey=null;
function gdiHomeCard(){
  const p=window.location.pathname;
  if(!/^\/(\d+:)?\/?$/.test(p))return;          // só na home
  if(!GDIUser.loaded())return;                   // espera o caderno carregar
  if(gdiHomeCardKey===p)return;                  // já montado nesta página
...
// ── NÍVEL 2b: painel de progresso da pasta aberta ──

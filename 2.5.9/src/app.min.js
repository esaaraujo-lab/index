        ...
        <div style="display:flex;gap:12px;overflow-x:auto;padding-bottom:8px;">
            ${continueWatching.map(item => `
                <div style="min-width:200px;background:rgba(0,0,0,0.2);border-radius:8px;padding:12px;cursor:pointer;" 
                     onclick="resumeVideo('${item.url}', ${item.progress})">
...
function resumeVideo(url, progress) {
    window.location.href = url;
    // O progresso será restaurado pelo sistema de salvamento de progresso
}
...
<div style="margin-top:10px;">
  <button class="gdi-btn gdi-btn-ghost" onclick="toggleNotesPanel()" title="Anotações">
    <i class="bi bi-journal-text"></i> Anotações
  </button>
</div>
${createNotesPanel('video_' + btoa(i).replace(/[^a-zA-Z0-9]/g, ''), i)}`;
...
// ============================================================
function createNotesPanel(videoId, videoName) {
    return `
    <div class="gdi-notes-panel" id="gdi-notes-panel" style="display:none;margin-top:16px;background:rgba(0,0,0,0.2);border-radius:8px;padding:16px;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">
            <strong style="font-size:14px;"><i class="bi bi-journal-text me-2"></i>Anotações</strong>
            <button class="gdi-btn gdi-btn-ghost" onclick="toggleNotesPanel()" title="Fechar">
                <i class="bi bi-x"></i>
            </button>
        </div>
        ...
        <div style="display:flex;gap:8px;">
            ...
            <button class="gdi-btn gdi-btn-primary" onclick="addVideoNote('${videoId}')" title="Adicionar nota">
...
function addVideoNote(videoId) {
    ...
    const savedNote = GDIUserSystem.addNote(videoId, note);
    if (savedNote) {
        input.value = '';
        loadNotes();
...
function loadNotes() {
    ...
    if (notes.length === 0) {
        notesList.innerHTML = '<p style="color:#8b949e;font-size:12px;">Nenhuma nota ainda.</p>';
        return;
    }
    
    notesList.innerHTML = notes.map(note => `
        <div class="gdi-note-item" style="display:flex;align-items:flex-start;justify-content:space-between;padding:8px;margin-bottom:6px;background:rgba(255,255,255,0.05);border-radius:4px;">
            <div style="flex:1;">
                ...
                <p style="margin:0;font-size:13px;">${escHtml(note.text)}</p>
            </div>
            <button onclick="removeVideoNote('${note.id}')" class="gdi-act-btn" title="Remover nota">
...
function removeVideoNote(noteId) {
    const videoId = getCurrentVideoId();
...
function setupVideoProgressTracking(videoId, videoName, videoUrl) {
    if (!window._gdiPlayerInstance) return;
    ...
    function saveProgress() {
        ...
        if (currentTime > 0 && duration > 0 && currentTime < duration - 5) {
    ...
    // Limpar intervalo quando a página for desmontada
...
`;
  ...
  `;
    ...
    function playAlarm(isBreak) {
        ...
        setTimeout(() => notes.forEach((f, i) => setTimeout(() => beep(f, 0.3, 'sine', 0.35), i * 280)), 1100);
    }
    ...
    function nextPhase() {
        ...
        if (phase === 'work') {
            ...
            playAlarm(true);
            notify(
                isLongBreak ? 'Pausa longa! 🛋' : 'Pausa! ☕',
                `${fmt(totalSecs)} de descanso. Você merece!`
            );
        } else {
            phase = 'work';
            totalSecs = cfg.work * 60;
    ...
    updateUI();
    console.log('[GDI Pomodoro] Initialized ✓');
})();

function initPomodoroOnce() {
    // No-op: pomodoro boots itself via IIFE above, domGuard keeps it alive
}

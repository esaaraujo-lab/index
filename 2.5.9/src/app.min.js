...
requestListPath(folderPath,{password:folderPw},async function(h){
  ...
  if(videos.length<=1){
    ...
    if(crossVideos.length>1){
      ...
      // FIX duplicata: no modo cross-folder o vídeo atual é sempre o representante
      // da pasta dele (só entra quando há ≤1 vídeo na pasta). Localiza pela PASTA —
      // o nome é renomeado ("001 - Pasta") e o streamUrl é regenerado a cada request
      // (AES-CBC com IV aleatório), então nunca batem por igualdade → o código antigo
      // não achava o vídeo e inseria uma cópia no topo (unshift).
...
// FIX: chave canônica SEM query (?a=view). Antes a chave mudava
// quando a playlist carregava (path vs path?a=view) → no reload
// a posição salva nunca era reencontrada.
// ============================================================
const GDIResume=(()=>{
...
// FIX: agora aguarda a metadata carregar (readyState≥1) antes de aplicar o seek —
// sem isso o browser IGNORA o currentTime e o guard antigo (lastKey) bloqueava
// a nova tentativa no loadedmetadata. Também re-aplica quando a chave muda
// (troca de faixa no APlayer / troca de vídeo na playlist).
function attachResumeTracking(media,getKey){
  if(!media||media.__gdiResume)return;
  media.__gdiResume=true;
  let lastSave=0,appliedKey=null;
  ...
  function applySeek(){
    ...
    if(media.readyState<1)return;               // aguarda metadata para o seek valer
    const d=dur();
    if(d&&sv.t>d-15){GDIResume.del(k);appliedKey=k;return}  // quase no fim = concluído
    try{
      media.currentTime=sv.t;
      appliedKey=k;
      showResumeToast(sv.t,()=>{try{media.currentTime=0}catch(e){}GDIResume.del(k)});
      console.log('[GDI Resume] retomando',k,'em',gdiFmtTime(sv.t));
    }catch(e){}
  }
...
//   • Sem badge de tempo no FAB
//   • Sem sync com o player (nada inicia ao dar play no vídeo)
//   • Sessão anterior restaura SEMPRE PAUSADA
// ============================================================
;(function(){
  function gdiPomBoot(){

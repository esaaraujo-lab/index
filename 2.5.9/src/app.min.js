...
const GDIUserSystem = {
    ...
    createDefaultUser() {
        return {
            ...
            preferences: {
                theme: 'dark',
                playerType: 'artplayer', // Alterado de plyr para artplayer
                autoplay: true,
                defaultVolume: 0.7,
    ...
    loadUserPreferences() {
        if (this.currentUser && this.currentUser.preferences) {
            ...
            if (window.player_config) {
                window.player_config.player = this.currentUser.preferences.playerType || 'artplayer';
            }
        }
    },
...
function generateBreadcrumb(i) {
    ...
    for (let a = 0; a < e.length; a++) {
        ...
        const o = d.match(/^(\d+):$/);
        const s = o ? window.drive_names && window.drive_names[+o[1]] || d : d || "Home";
        const r = s.length > 20 ? s.slice(0, 16) + "\u2026" : s;
        if (l) {
            t += `<li class="gdi-bc-cur" title="${escHtml(s)}">${escHtml(r)}</li>`;
        } else {
            t += `<li><a href="${n ? n + "/" : "/"}" title="${escHtml(s)}">${escHtml(r)}</a></li><li class="gdi-bc-sep">/</li>`;
        }
    }
    return t;
}
...
function formatDateTime(i) {
    return i ? new Date(i).toLocaleString() : "";
}
...
const GDIDebug = (() => {
    ...
    function n() {
        ...
        const o = i.map(r => {
            const p = d[r.type] || "#aaa";
            const g = r.data != null ? typeof r.data == "string" ? r.data : JSON.stringify(r.data, null, 2) : "";
            return `<div class="gdi-dbg-entry"><span class="gdi-dbg-ts">${r.ts}</span><span class="gdi-dbg-badge" style="color:${p}">[${r.type.toUpperCase()}]</span><span class="gdi-dbg-msg">${escHtml(r.label)}</span>` +
                   (g ? `<pre class="gdi-dbg-pre">${escHtml(g)}</pre>` : "") + "</div>";
        }).join("");
        e.innerHTML = o || '<span class="gdi-dbg-empty">No entries yet.</span>';
        e.scrollTop = e.scrollHeight;
        const s = document.getElementById("gdi-dbg-count");
        s && (s.textContent = i.length);
    }
    
    function a(d, o, s) {
        window.UI?.debug_mode && (i.push({ts: t(), type: d, label: o, data: s !== void 0 ? s : null}), n());
    }
    ...
    function c() {
        ...
        i.length > 0 && n();
        a("info", "Debug attached", {path: window.location.pathname, search: window.location.search, drive: window.current_drive_order, version: window.UI?.version, model_type: window.MODEL?.root_type});
    }
    
    function l() {
        i.length = 0;
        e && (e.innerHTML = '<span class="gdi-dbg-empty">Cleared.</span>');
        const d = document.getElementById("gdi-dbg-count");
        d && (d.textContent = "0");
    }
    
    return {log: a, attach: c, clear: l};
})();

// ============================================================
// INTERCEPTAÇÃO DE FETCH PARA DEBUG
// ============================================================
if (window.UI?.debug_mode) {
    const originalFetch = window.fetch.bind(window);
    window.fetch = async function(t, n) {
        const url = typeof t == "string" ? t : t.url || String(t);
        const method = (n?.method || "GET").toUpperCase();
        let body;
        try { body = n?.body ? JSON.parse(n.body) : void 0; } catch { body = n?.body; }
        GDIDebug.log("req", `→ ${method} ${url}`, body !== void 0 ? body : null);
        const startTime = Date.now();
        try {
            const response = await originalFetch(t, n);
            const clone = response.clone();
            let data;
            try { data = await clone.json(); } catch { data = null; }
            GDIDebug.log(response.ok ? "api" : "error", `← ${response.status} ${url} (${Date.now() - startTime}ms)`, data);
            return response;
        } catch (error) {
            GDIDebug.log("error", `✗ FETCH FAILED: ${url}`, String(error));
            throw error;
        }
    };
    ...
    console.error = function(...args) {
        GDIDebug.log("error", args.map(arg => 
...
function title(i) {
    ...
    n.is_search_page ? $("title").html(`${t} - Search: ${n.q || ""}`) : $("title").html(`${t} - ${i}`);
}
...
function nav(i) {
    ...
    let l = "";
    a.forEach((r, p) => {
        l += `<li><a class="dropdown-item${p === t ? " active" : ""}" href="/${p}:/">
          <i class="bi bi-folder2-open"></i> ${r}</a></li>`;
    });
    
    const d = UI.logo_image ? `<img src="${UI.logo_link_name}" alt="${UI.company_name}" height="28">` : `<i class="bi bi-cloud-fill"></i> ${UI.logo_link_name}`;
...
function requestListPath(i, e, t, n, a = 3, c = false) {
    ...
    async function o(s) {
        try {
            ...
            if (s > 0) {
                $("#update").html(`<div class="gdi-alert gdi-alert-info">Retrying… (${s} left)</div>`);
                await sleep(2000);
                await o(s - 1);
            } else {
                $("#update").html('<div class="gdi-alert gdi-alert-error">Unable to connect. Please try again.</div>');
                $("#list").html(`<div class="gdi-empty"><i class="bi bi-wifi-off"></i><p>${escHtml(String(r))}</p></div>`);
                $("#update").hide();
            }
        }
    }
...
function requestSearch(i, e, t = 3) {
    const n = {
        q: i.q || null,
    ...
    async function a(c) {
        try {
            const l = await fetch(`/${window.current_drive_order || 0}:search`, {
                method: "POST",
                headers: {"Content-Type": "application/json"},
                body: JSON.stringify(n)
            });
            if (!l.ok) throw new Error("Request failed with status " + l.status);
            ...
            if (c > 0) {
                $("#update").html(`<div class="gdi-alert gdi-alert-info">Retrying… (${c} left)</div>`);
                await sleep(2000);
                await a(c - 1);
            } else {
                $("#update").html(`<div class="gdi-alert gdi-alert-error">Unable to connect after ${t} attempts.</div>`);
...
function list(i, e = "", t = false) {
    ...
    let c = "/";
    let l = '<li><a href="/">Home</a></li><li class="gdi-bc-sep">/</li>';
    ...
    // Adicionar banner "Continuar Assistindo"
            ...
            if (window.scroll_status.event_bound !== true) {
                $(window).on("scroll", function() {
                    if ($(this).scrollTop() + $(this).height() > getDocumentHeight() - (Os.isMobile ? 130 : 80)) {
                        ...
                        }, s, null, 5, true) : requestListPath(g, {
                            password: f.password,
                            page_token: u.data("nextPageToken"),
                            page_index: u.data("curPageIndex") + 1
                        }, s, null);
    ...
    const r = document.getElementById("handle-multiple-items-copy");
    r && r.addEventListener("click", () => {
        const p = document.querySelectorAll("input.gdi-row-check:checked");
        if (!p.length) {
            alert("No items selected!");
            return;
        }
        const g = Array.from(p).map(f => f.value).join("\n");
        ...
        showToast(`${p.length} link${p.length > 1 ? "s" : ""} copied`);
    });
}

// Banner "Continuar Assistindo"
function addContinueWatchingBanner() {
    const continueWatching = GDIUserSystem.getContinueWatching();
    if (continueWatching.length === 0) return;
    
    const bannerHTML = `
    <div class="gdi-continue-watching" style="margin-bottom:16px;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">
            <strong style="font-size:14px;"><i class="bi bi-clock-history me-2"></i>Continuar Assistindo</strong>
        </div>
        <div style="display:flex;gap:12px;overflow-x:auto;padding-bottom:8px;">
            ${continueWatching.map(item => `
                <div style="min-width:200px;background:rgba(0,0,0,0.2);border-radius:8px;padding:12px;cursor:pointer;" 
                     onclick="resumeVideo('${item.videoId}')">
                    <div style="font-size:13px;font-weight:600;margin-bottom:8px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
                        ${escHtml(item.name)}
                    </div>
                    <div style="height:4px;background:rgba(255,255,255,0.1);border-radius:2px;overflow:hidden;margin-bottom:8px;">
                        <div style="height:100%;width:${item.progress * 100}%;background:#1f6feb;border-radius:2px;"></div>
                    </div>
    ...
    const breadcrumb = document.querySelector('.gdi-breadcrumb-wrap');
    if (breadcrumb && !document.querySelector('.gdi-continue-watching')) {
        breadcrumb.insertAdjacentHTML('beforebegin', bannerHTML);
    }
}
...
function askPassword(i) {
    $("#spinner").remove();
    const e = prompt("This folder is password protected. Enter the password:", "");
    e != null && e != "" ? (localStorage.setItem("password" + i, e), list(i)) : history.go(-1);
}

let _folderFilterBound = false;
function initFolderFilter() {
    const i = document.getElementById("folder-filter");
    !i || _folderFilterBound || (_folderFilterBound = true, i.addEventListener("input", function() {
        const e = this.value.toLowerCase();
        document.querySelectorAll("#list .gdi-row").forEach(t => {
            const n = (t.dataset.name || t.textContent).toLowerCase();
...
function initColumnSort() {
    ...
    i.forEach(e => {
        e.addEventListener("click", function() {
            const t = this.dataset.sort;
            _sortState.col === t ? _sortState.dir *= -1 : (_sortState.col = t, _sortState.dir = 1);
            i.forEach(n => n.classList.remove("asc", "desc"));
            this.classList.add(_sortState.dir === 1 ? "asc" : "desc");
            sessionStorage.setItem("gdi-sort", JSON.stringify(_sortState));
            sortFileList(t, _sortState.dir);
        });
    });
    
    try {
        const e = JSON.parse(sessionStorage.getItem("gdi-sort"));
        e && e.col && (_sortState = e);
    } catch {}
}

function sortFileList(i, e) {
    const t = $("#list");
    const n = t.children(".gdi-row").toArray();
    n.sort((a, c) => {
        if (i === "size") return e * ((parseFloat($(a).data("bytes")) || 0) - (parseFloat($(c).data("bytes")) || 0));
        if (i === "date") return e * (new Date($(a).data("date")) - new Date($(c).data("date")));
        const l = ($(a).data("name") || "").toLowerCase();
        const d = ($(c).data("name") || "").toLowerCase();
        return e * l.localeCompare(d);
    });
    n.forEach(a => t.append(a));
}

function append_files_to_list(i, e) {
    const UI = window.UI || {};
    const t = $("#list");
    const n = t.data("nextPageToken") === null;
    const a = t.data("curPageIndex") == "0";
    let c = "", l = 0, d = false;
    ...
    for (const o in e) {
        ...
        if (s.modifiedTime = utc2delhi(s.modifiedTime), s.mimeType == "application/vnd.google-apps.folder") {
            c += `<a href="${p}" class="gdi-row countitems" data-name="${escHtml(s.name.toLowerCase())}" data-date="${s.modifiedTime || ""}">
  ...
  <span class="gdi-row-acts"></span>
            ...
            if (n && s.name == "README.md" && UI.render_readme_md) {
                get_file(v, s, function(b) {
                    markdown("#readme_md", b);
                    $("img").addClass("img-fluid");
                });
            }
            
            if (s.name == "HEAD.md" && UI.render_head_md) {
                get_file(v, s, function(b) {
                    markdown("#head_md", b);
                    $("img").addClass("img-fluid");
                });
            ...
            const w = GDOC_TYPES[s.mimeType];
            const y = w ? w.icon : getFileIcon(f);
            const _ = w && UI.display_download ? w.formats.map(b => 
                `<a class="gdi-act-btn" href="${u}&fmt=${b.ext}" title="Export as ${b.label}" download><span style="font-size:10px;font-weight:600;">${b.label}</span></a>`
            ).join("") : "";
            
            c += `<div class="gdi-row countitems size_items" data-name="${escHtml(s.name.toLowerCase())}" data-bytes="${g}" data-date="${s.modifiedTime || ""}">
  ...
  <span class="gdi-row-acts">
    ...
    if (d && UI.allow_selecting_files) {
        document.getElementById("select_items").style.display = "flex";
    }
    
    t.html((t.data("curPageIndex") == "0" ? "" : t.html()) + c);
    _folderFilterBound = false;
    initFolderFilter();
    initColumnSort();
    
    if (n) {
        const o = formatFileSize(l) || "0 Bytes";
        const s = t.find(".countitems").length;
        const r = t.find(".size_items").length;
        const p = s === 0 ? "Empty folder" : `${s} item${s === 1 ? "" : "s"}`;
        const g = r > 0 ? ` &middot; ${r} file${r === 1 ? "" : "s"}, ${o}` : "";
...
function append_files_to_fallback_list(i, e) {
    ...
    try {
        ...
        for (const d in e) {
            const o = e[d];
            const s = "/fallback?id=" + encodeURIComponent(o.id);
            
            if (o.modifiedTime = utc2delhi(o.modifiedTime), o.mimeType == "application/vnd.google-apps.folder") {
                a += `<a href="${s}" class="gdi-row countitems" data-name="${escHtml(o.name.toLowerCase())}" data-date="${o.modifiedTime || ""}">
  <span class="gdi-row-icon"><i class="bi bi-folder-fill gdi-icon-folder"></i></span>
  ...
  <span class="gdi-row-acts"></span>
                ...
                const r = Number(o.size);
                c += r;
                o.size = formatFileSize(o.size);
                l = true;
                const p = o.fileExtension;
                const g = UI.second_domain_for_dl ? UI.downloaddomain + o.link : window.location.origin + o.link;
                const f = s + "&a=view";
                
                if (n && o.name == "README.md" && UI.render_readme_md) {
                    get_file(s, o, function(u) {
                        markdown("#readme_md", u);
                        $("img").addClass("img-fluid");
                    });
                }
                
                if (o.name == "HEAD.md" && UI.render_head_md) {
                    get_file(s, o, function(u) {
                        markdown("#head_md", u);
                        $("img").addClass("img-fluid");
                    });
                ...
                a += `<div class="gdi-row countitems size_items" data-name="${escHtml(o.name.toLowerCase())}" data-bytes="${r}" data-date="${o.modifiedTime || ""}">
  ...
  <span class="gdi-row-acts">
    ...
    ${UI.display_download ? `<a class="gdi-act-btn" href="${g}" title="Download"><i class="bi bi-download"></i></a>` : ""}
        ...
        if (l && UI.allow_selecting_files) {
            document.getElementById("select_items").style.display = "flex";
        }
        
        t.html((t.data("curPageIndex") == "0" ? "" : t.html()) + a);
        
        if (n) {
            const d = formatFileSize(c) || "0 Bytes";
            const o = t.find(".countitems").length;
            const s = t.find(".size_items").length;
            const r = o === 0 ? "Empty folder" : `${o} item${o === 1 ? "" : "s"}`;
            const p = s > 0 ? ` &middot; ${s} file${s === 1 ? "" : "s"}, ${d}` : "";
            $("#count").addClass("show").html(r + p);
        }
    } catch (t) {
        console.error(t);
    }
...
function render_search_result_list() {
    ...
    function t(a, c) {
        $("#list").data("nextPageToken", a.nextPageToken).data("curPageIndex", a.curPageIndex);
        $("#spinner").remove();
        ...
        if (a.nextPageToken === null) {
            ...
            window.scroll_status.event_bound = false;
            window.scroll_status.loading_lock = false;
            append_search_result_to_list(a.data.files);
        } else {
            append_search_result_to_list(a.data.files);
            ...
            if (window.scroll_status.event_bound !== true) {
                $(window).on("scroll", function() {
                    if ($(this).scrollTop() + $(this).height() > getDocumentHeight() - (Os.isMobile ? 130 : 80)) {
                        ...
                        const l = $("#list");
                        requestSearch({q: i, page_token: l.data("nextPageToken"), page_index: l.data("curPageIndex") + 1}, t);
                    }
                });
                window.scroll_status.event_bound = true;
            }
    ...
    const n = document.getElementById("handle-multiple-items-copy");
    n && n.addEventListener("click", () => {
        const a = document.querySelectorAll("input.gdi-row-check:checked");
        if (!a.length) {
            alert("No items selected!");
            return;
        }
        const c = Array.from(a).map(l => l.value).join("\n");
        ...
        showToast(`${a.length} link${a.length > 1 ? "s" : ""} copied`);
    });
}

function append_search_result_to_list(i) {
    const UI = window.UI || {};
    try {
        const e = $("#list");
        const t = e.data("nextPageToken") === null;
        let n = "", a = 0, c = false;
        
        for (const l in i) {
            const d = i[l];
            d.size == null && (d.size = "");
            d.modifiedTime = utc2delhi(d.modifiedTime);
            const o = typeof d.rootIdx == "number" ? d.rootIdx : -1;
            
            if (d.mimeType == "application/vnd.google-apps.folder") {
                n += `<a style="cursor:pointer;" onclick="onSearchResultItemClick('${escHtml(d.id)}', false, ${o})" data-bs-toggle="modal" data-bs-target="#SearchModel"
  ...
  <span class="gdi-row-acts"></span>
                ...
                const r = d.fileExtension;
                const p = UI.second_domain_for_dl ? UI.downloaddomain + d.link : window.location.origin + d.link;
                
                n += `<div class="gdi-row countitems size_items" data-name="${escHtml(d.name.toLowerCase())}" data-bytes="${s}" data-date="${d.modifiedTime || ""}">
  ...
  <span class="gdi-row-acts">
    ${UI.display_download ? `<a class="gdi-act-btn" href="${p}" title="Download"><i class="bi bi-download"></i></a>` : ""}
        ...
        if (c && UI.allow_selecting_files) {
            document.getElementById("select_items").style.display = "flex";
        }
        
        e.html((e.data("curPageIndex") == "0" ? "" : e.html()) + n);
        
        if (t) {
            const l = formatFileSize(a) || "0 Bytes";
            const d = e.find(".countitems").length;
            const o = e.find(".size_items").length;
            
            if (d === 0) {
                $("#count").addClass("show").html("No results found");
            } else {
                const s = `${d} result${d === 1 ? "" : "s"}`;
                const r = o > 0 ? ` &middot; ${l}` : "";
                $("#count").addClass("show").html(s + r);
            }
        }
    } catch (e) {
        console.error(e);
...
function onSearchResultItemClick(i, e, t) {
    const n = `/fallback?id=${encodeURIComponent(i)}${e ? "&a=view" : ""}`;
    
    function a() {
        const d = document.getElementById("SearchModel");
        const o = d && window.bootstrap?.Modal?.getInstance(d);
        o && o.hide();
    ...
    async function l() {
        try {
            ...
            if (d.ok) {
                ...
                if (o.path) {
                    const s = o.path.replace(/#/g, "%23").replace(/\?/g, "%3F");
                    $("#SearchModelLabel").html("Open file");
                    $("#modal-body-space").html(`
                      <a class="gdi-btn gdi-btn-primary me-2" href="${s}${e ? "?a=view" : ""}">Open</a>
                      <a class="gdi-btn gdi-btn-ghost" href="${s}${e ? "?a=view" : ""}" target="_blank">Open in new tab</a>`);
                    return;
                }
            }
        } catch {}
...
function get_file(i, e, t) {
    const n = "file_path_" + i + e.modifiedTime;
...
async function fallback(i, e) {
    if (e) {
        ...
        fetch("/0:fallback", {
            ...
            return n.json();
        }).then(n => dispatchFileView(n, t)).catch(n => {
            $("#content").html(renderErrorCard(n));
        });
    } else {
        return list(i, true);
    }
...
async function file(i) {
    ...
    fetch("", {
        ...
        return t.json();
    }).then(t => dispatchFileView(t, e)).catch(t => {
        $("#content").html(renderErrorCard(t));
    });
}
...
function dispatchFileView(i, e) {
    ...
    } else if (FILE_TYPES.video.includes(n) || t && t.includes("video")) {
        const s = i.thumbnailLink ? i.thumbnailLink.replace("s220", "s0") : UI.poster;
        file_video(a, c, l, s, d, t, o, e);
    } else if (FILE_TYPES.audio.includes(n) || t && t.includes("audio")) {
        file_audio(a, c, l, d, o, e);
    } else if (FILE_TYPES.image.includes(n) || t && t.includes("image")) {
        file_image(a, c, l, d, o, e);
    } else if (n === "pdf" || t && t.includes("pdf")) {
...
function renderDownloadButtons(i, e, t = {}) {
    const n = btoa(i);
    const a = t.showMedia ? `
...
function copyShareUrl(i) {
    navigator.clipboard.writeText(i).then(() => showToast("Link copied!")).catch(() => {
...
function showToast(i) {
    ...
    e || (e = document.createElement("div"), e.id = "gdi-toast-container", document.body.appendChild(e));
    
    const t = document.createElement("div");
    t.className = "gdi-toast";
    t.innerHTML = `<i class="bi bi-check-circle-fill"></i> ${i}`;
    e.appendChild(t);
...
function _viewerCard(i, e, t, n, a) {
    return `<div class="gdi-wrap">
  <div class="gdi-viewer">
    <div class="gdi-breadcrumb-wrap">
...
function file_others(i, e, t, n, a, c) {
    const l = getFileIcon(i.split(".").pop());
    $("#content").html(_viewerCard(l, i, t, '<p class="mb-3" style="color:var(--gdi-text-muted);font-size:13px;">No preview available for this file type.</p>', renderDownloadButtons(n, e)));
}

function file_workspace(i, e, t, n) {
    const a = GDOC_TYPES[t];
    const c = a ? a.icon : '<i class="bi bi-file-earmark-text"></i>';
    const l = a ? a.name : "Workspace File";
    const o = (a ? a.formats : []).map(r => 
        `<a class="btn btn-sm btn-outline-secondary me-2 mb-2" href="${n}&fmt=${r.ext}" download="${escHtml(i)}.${r.ext}">
    ...
    $("#content").html(_viewerCard(c, i, e, s, ""));
}

function file_code(i, e, t, n, a, c, l, d) {
    const UI = window.UI || {};
    const o = getFileIcon(c);
    
    $("#content").html(_viewerCard(o, i, t, `
    <div id="code_spinner"></div>
    <div class="gdi-code-outer" style="display:none;">
      <pre><code id="editor"></code></pre>
    </div>`, renderDownloadButtons(a, e)));
    
    UI.second_domain_for_dl || ($("#code_spinner").html('<div class="gdi-spinner-wrap"><div class="gdi-spinner"></div></div>'), n <= 1024 * 1024 * 2 ? $.get(a, function(r) {
        $("#editor").html($("<div/>").text(r).html());
        $("#code_spinner").remove();
        $(".gdi-code-outer").show();
    }) : ($("#code_spinner").remove(), $(".gdi-code-outer").show(), $("#editor").html('<span style="color:var(--gdi-text-muted);">File too large to preview (max 2 MB)</span>')));
}

// ============================================================
// PLAYER DE VÍDEO COM TODAS AS FUNCIONALIDADES
// ============================================================
function file_video(i, e, t, n, a, c, l, d, o) {
    ...
    if (!UI.disable_player) {
      ...
      ${o.map(h => `<track kind="subtitles" src="${escHtml(h.url)}" label="${escHtml(h.label)}" default>`).join("")}
            ...
            r = '<div id="player-container" style="width:100%;aspect-ratio:16/9;background:#000;"></div>';
            p = "https://cdn.jsdelivr.net/npm/artplayer/dist/artplayer.js";
        }
    }
    
    // Botão e painel de anotações
    const notesButton = `
    <button class="gdi-btn gdi-btn-ghost" onclick="toggleNotesPanel()" 
            style="margin-top:10px;width:100%;" title="Anotações">
        <i class="bi bi-journal-text"></i> Anotações
    </button>`;
    
    const notesPanel = `
    <div id="gdi-notes-panel" style="display:none;margin-top:10px;background:rgba(0,0,0,0.2);border-radius:8px;padding:16px;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
            <strong style="font-size:14px;">
                <i class="bi bi-journal-text me-2"></i>Anotações
            </strong>
            <button onclick="toggleNotesPanel()" style="background:none;border:none;color:#8b949e;font-size:20px;cursor:pointer;">
                &times;
            </button>
        </div>
        ...
        <div style="display:flex;gap:8px;">
            ...
            <button class="gdi-btn gdi-btn-primary" onclick="addVideoNote()" title="Adicionar nota">
...
${notesPanel}`;
    ...
    function enableAutoplayWithUnmute() {
        ...
        const attemptUnmute = setInterval(() => {
            ...
            if (!player || unmuteAttempts >= maxAttempts) {
                clearInterval(attemptUnmute);
                return;
            }
            
            try {
                if (typeof player.muted === "boolean") {
                    player.muted = false;
                } else if (typeof player.muted === "function") {
                    player.muted(false);
                } else if (typeof player.setMute === "function") {
    ...
    function initPlayerEvents() {
        if (player_config.player === "plyr") {
            ...
            m.on("ready", () => {
                m.play().catch(() => {});
                setTimeout(() => { m.muted = false; }, 2000);
                setupVideoProgressTracking(getCurrentVideoId(), i, window.location.href);
            });
            ...
            m.ready(function() {
                ...
                this.el().addEventListener("keydown", function(v) {
                    ...
                    if (v.key === " ") {
                        ...
                        m.volume(Math.min(1, m.volume() + .1));
                    } else if (v.key === "ArrowDown") {
                        m.volume(Math.max(0, m.volume() - .1));
                    }
                });
            });
            ...
            const m = new Artplayer({
                ...
                airplay: true,
                theme: '#1f6feb',
                subtitle: o.length ? {url: o[0].url, type: 'webvtt'} : {}
            });
            setTimeout(() => { m.muted = false; }, 2000);
            window._gdiPlayerInstance = m;
            m.on('video:ended', playNextInPlaylist);
            setupVideoProgressTracking(getCurrentVideoId(), i, window.location.href);
        }
    }
    
    if (p) {
        if (window.Plyr || window.videojs || window.DPlayer || window.jwplayer || window.Artplayer) {
            initPlayerEvents();
            enableAutoplayWithUnmute();
        } else {
    ...
    function switchVideo(h) {
        ...
        if (player_config.player === "plyr" && window._gdiPlayerInstance) {
            ...
            window._gdiPlayerInstance.load([{file: m.streamUrl, type: m.mimeType, image: m.poster, title: m.name}]);
            window._gdiPlayerInstance.play();
        } else if (player_config.player === "artplayer" && window._gdiPlayerInstance) {
            window._gdiPlayerInstance.switchUrl(m.streamUrl, m.name);
            window._gdiPlayerInstance.poster = m.poster;
            window._gdiPlayerInstance.play();
        } else {
    ...
    async function loadCrossFolderPlaylist() {
        return new Promise((resolve) => {
            requestListPath(parentPath, {}, function(parentData) {
                if (!parentData || !parentData.data || !parentData.data.files) {
                    resolve([]);
                    return;
                }
                ...
                subFolders.forEach(folder => {
                    ...
                    requestListPath(fpath, {}, function(folderData) {
                        if (folderData && folderData.data && folderData.data.files) {
                            const vids = buildPlaylistFromFiles(folderData.data.files, fpath, folder.name);
                            if (vids.length > 0) {
                                allVideos.push({sortKey: folder.name, folderName: folder.name, video: vids[0]});
                            }
                        }
                        ...
                        if (done === subFolders.length) {
                            ...
                            const videos = allVideos.map(x => x.video);
                            
                            if (videos.length > 1) {
                                const nameCounts = {};
                                videos.forEach(v => { nameCounts[v.name] = (nameCounts[v.name] || 0) + 1; });
                                const maxCount = Math.max(...Object.values(nameCounts));
                                
                                if (maxCount / videos.length > 0.3) {
                                    allVideos.forEach((item, idx) => {
                                        const num = String(idx + 1).padStart(3, '0');
                                        item.video.name = num + ' - ' + item.folderName;
                                    });
                                }
                            }
...
function loadVideoNotes() {
    ...
    if (notes.length === 0) {
        notesList.innerHTML = '<p style="color:#8b949e;font-size:12px;text-align:center;">Nenhuma anotação ainda</p>';
        return;
    }
    
    notesList.innerHTML = notes.map(note => `
        <div style="display:flex;align-items:flex-start;justify-content:space-between;padding:8px;margin-bottom:6px;background:rgba(255,255,255,0.05);border-radius:4px;">
            <div style="flex:1;">
                <button onclick="jumpToTimestamp(${note.timestamp})" 
                        style="background:none;border:none;color:#74c0fc;cursor:pointer;font-size:11px;padding:0;margin-bottom:4px;">
                    <i class="bi bi-play-fill"></i> ${note.formattedTime}
                </button>
                <p style="margin:0;font-size:13px;color:#e6edf3;">${escHtml(note.text)}</p>
            </div>
            <button onclick="deleteVideoNote('${note.id}')" 
                    style="background:none;border:none;color:#8b949e;cursor:pointer;font-size:12px;" 
                    title="Remover nota">
...
function getCurrentVideoId() {
    const videoName = document.querySelector('.gdi-file-header-name')?.textContent;
    if (videoName) {
        return 'video_' + btoa(videoName).replace(/[^a-zA-Z0-9]/g, '');
    }
...
function setupVideoProgressTracking(videoId, videoName, videoUrl) {
    ...
    function getDuration() {
        ...
        } else if (typeof player.duration !== 'undefined') {
            return player.duration;
        }
        return 0;
    }
...
// ============================================================
// PLAYER DE ÁUDIO COM MODO SLEEP
// ============================================================
...
// ============================================================
// MODO SLEEP PARA ÁUDIO
// ============================================================
function initAudioSleepMode() {
    const existingOverlay = document.getElementById('gdi-sleep-overlay');
...
// ============================================================
function file_image(i, e, t, n, a, c) {
    const l = `<div class="gdi-wrap">
  <div class="gdi-viewer">
    <div class="gdi-breadcrumb-wrap"><ol class="gdi-bc">${_viewerBreadcrumb()}</ol></div>
    <div class="gdi-viewer-card">
...
`;
  ...
  `;
    ...
    function buildDots() {
        ...
        let h = '';
        const sessionsInCycle = completedSessions % cfg.sessions;
    ...
    function nextPhase() {
        ...
        if (phase === 'work') {
            completedSessions++;
            const isLongBreak = completedSessions % cfg.sessions === 0;
            phase = isLongBreak ? 'long' : 'short';
            totalSecs = (isLongBreak ? cfg.long : cfg.short) * 60;
    ...
    function syncToPlayer(videoEl) {
        ...
        videoEl.addEventListener('ended', onVideoPause);
        ...
        const cleanupObserver = new MutationObserver(() => {
            if (!document.body.contains(videoEl)) {
                ...
                videoEl.removeEventListener('ended', onVideoPause);
                playerSynced = false;
...
document.addEventListener("change", function(i) {
    i.target && i.target.id === "select-all-checkboxes" && document.querySelectorAll("input.gdi-row-check").forEach(t => {
...
function fetchQuota() {
    ...
    fetch(`/${i}:quota`).then(e => {
        if (!e.ok) throw new Error("quota fetch failed");
        return e.json();
    }).then(e => {
        const t = e.storageQuota;
        if (!t) return;
        const n = Number(t.usage || 0);
        const a = Number(t.limit || 0);
        const c = document.getElementById("gdi-quota-bar");
        const l = document.getElementById("gdi-quota-text");
        const d = document.getElementById("gdi-quota-fill");
        if (!c || !l || !d) return;
        
        const o = a > 0 ? Math.min(100, n / a * 100) : 0;
        const s = o > 90 ? "#f44336" : o > 70 ? "#ff9800" : "#4caf50";
        
        l.textContent = a > 0 ? `${formatFileSize(n)} used of ${formatFileSize(a)} (${o.toFixed(1)}%)` : `${formatFileSize(n)} used`;
        d.style.width = o + "%";
...
$(function() {
    ...
    console.log('[GDI] Aplicação inicializada com sucesso');
});
...
with open("app-modern-player.min.js.txt", "w", encoding="utf-8") as f:
    f.write(app_js_code)

print("[file-tag: app-modern-player.min.js.txt]")

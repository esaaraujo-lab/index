...
// ═══════════════════════════════════════════════════════════════
// PLAYER + MODO ESTUDO (vídeo) — v18: N1 autoplay c/ cancelar +
// N3 pular intro POR CURSO (pasta do vídeo), sincronizado na conta
// ═══════════════════════════════════════════════════════════════
...
// Cursos diferentes dentro do mesmo drive têm tempos diferentes.
...
// ═══ N1: AUTOPLAY COM CONTAGEM (estilo Netflix, com cancelar) ═══
...
Bus.on('media:ready',({type,el})=>{
  ...
  // v18.1: zera o cronômetro quando o NOVO vídeo terminar de carregar
  // (antes ele mostrava o tempo residual da aula anterior, ex.: 41:52)
...
// 'ended' dos 4 players passa pelo overlay "A seguir"
...
function switchVideo(h){
  ...
  // v18.1: zera o cronômetro imediatamente (mostrava o tempo residual da aula anterior)
  const ntEl=document.getElementById('gdi-note-time');
  if(ntEl)ntEl.textContent='00:00';
  try{const v=document.querySelector('.gdi-player-wrap video');if(v&&isFinite(v.currentTime)&&v.currentTime>0.3)v.currentTime=0;}catch(_){}
  renderPlaylistUI();

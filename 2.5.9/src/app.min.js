...
// ─────────────────────────────────────────────────────────────
// FIX PLAYLIST: busca TODAS as páginas de uma pasta (Drive API
// retorna no máximo 100 itens/página — pastas com vídeos+PDFs
// estavam cortando a playlist e escondendo o vídeo atual).
// ─────────────────────────────────────────────────────────────
...
// Remove duplicatas (mesma pasta + mesmo nome) que possam vir de listagens paginadas
...
async function loadCrossFolderPlaylist(){
  return new Promise(async (resolve)=>{
    ...
    // FIX: percorre TODAS as páginas da pasta pai (senão subpastas somem em pastas grandes)
    const parentFiles=await gdiListAllFiles(parentPath,parentPw);
    let subFolders=parentFiles.filter(f=>f.mimeType==="application/vnd.google-apps.folder");
    if(!subFolders.length){resolve([]);return;}
    if(subFolders.length>60)subFolders=subFolders.slice(0,60);
    ...
    subFolders.forEach(async folder=>{
      ...
      // FIX: todas as páginas da subpasta — o 1º vídeo pode estar além da 1ª página (PDFs antes)
      const folderFiles=await gdiListAllFiles(fpath,fpw);
      const vids=buildPlaylistFromFiles(folderFiles,fpath,folder.name);
...
(async function buildPlaylist(){
  ...
  // FIX PRINCIPAL: TODAS as páginas da pasta atual. Antes lia só a 1ª página
  // (100 itens); em pastas com vídeos+PDFs, os vídeos finais (041+) ficavam na
  // 2ª página → não eram encontrados → unshift criava a duplicata "001 - aula.mp4"
  // e a playlist parava no fim da 1ª página.
  ...
  if(videos.length<=1){
    ...
    if(crossVideos.length>1){
      ...
      // Cross-folder: o vídeo atual é o representante da pasta dele.
      const gdiNormPath=p=>{try{return decodeURIComponent(String(p||'')).replace(/\/+$/,'')}catch(_){return String(p||'').replace(/\/+$/,'')}};
      currentIndex=playlistVideos.findIndex(v=>gdiNormPath(v.folder)===gdiNormPath(folderPath)||v.name===i||v.streamUrl===a);
      console.log('[GDI Playlist] cross-folder:',playlistVideos.length,'itens, currentIndex=',currentIndex);
      if(currentIndex===-1){
...
// Reforçado: polling de segurança (tenta o seek por até 6s cobrindo qualquer
// timing do Plyr/VideoJS) + logs no Console para diagnóstico em produção.
function attachResumeTracking(media,getKey){
  ...
  // Polling: cobre qualquer timing de player/CDN (até 6s)
...
// ============================================================
// MODO DESCANSO — compartilhado entre áudio e vídeo
//   áudio : automático após 8s tocando + botão manual
//   vídeo : SOMENTE manual (botão 🌙) — clique na tela para voltar
// ============================================================
...
`;
    ...
    // Restaura tempo/dots de sessão anterior SEMPRE PAUSADO — nada roda sem clique
    (function restore(){

...
console.log('[GDI app] build v15.0-full');

// ═══════════════════════════════════════════════════════════════
// v15.0 — MÓDULO 01: CSS global único
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// MÓDULO 02: utilitários + Bus de eventos
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// MÓDULO 04: listagem de pastas — D3: cache temporário (45s).
// Playlist e Materiais pedem a MESMA pasta; agora a 2ª pergunta é
// instantânea. O cache zera a cada navegação (dados sempre frescos
// ao trocar de página).
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// MÓDULO 05: shell, navegação, roteamento
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// MÓDULO 06: listas unificadas (pasta / fallback / busca)
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// MÓDULO 07: PLAYER + MODO ESTUDO (vídeo)
// ★ CORREÇÃO PRINCIPAL v15: o ouvinte de 'media:ready' agora é
//   registrado ANTES do anúncio do vídeo (a ordem antiga fazia o
//   anúncio acontecer antes de alguém escutar — cronômetro parado,
//   retomada morta, anotações que não carregavam).
// ═══════════════════════════════════════════════════════════════
...
// ★ A1: trocou de aula com rascunho não salvo? descarta com aviso
//   (a nota NUNCA mais vai parar na aula errada)
...
// ★ OUVINTE registrado ANTES de qualquer anúncio (corrige o bug)
Bus.on('media:ready',({type,el})=>{
  ...
  // ★ clique no tempo do cronômetro → pula o vídeo para esse momento
...
// ★ Anúncio com RETRY: começa depois que file_video termina (evita
//   variáveis ainda não declaradas) e tenta até achar o <video>
//   (cobre players que criam o elemento de forma assíncrona)
...
function switchVideo(h){
  ...
  $("title").html(`${v} - ${m.name}`);
  Bus.emit('title:change'); // ★ A2: Pomodoro atualiza o "título base"
  try{GDIUser.pushHistory((m.pageUrl||window.location.pathname).split('?')[0],m.name);}catch(_){}
...
try{GDIUser.pushHistory(window.location.pathname,i);}catch(_){} // ★ C3
...
// ── MATERIAIS (por aula, com geração anti-mistura e iframes reaproveitados) ──
...
// ★ Redundância: anotações/assistido carregam MESMO se o anúncio do
//   vídeo falhar por qualquer motivo (antes ficavam em "Carregando…")
...
// ═══════════════════════════════════════════════════════════════
// MÓDULO 09: utilitários de formato + boot
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// MÓDULO 10: ESTADO DO USUÁRIO v3
// • B2: ao salvar, o site primeiro RELÊ a versão do servidor e
//   JUNTA as duas (o mais recente vence por item) — usar celular e
//   PC não se sobrescreve mais.
// • C3: histórico das últimas aulas abertas (aparece na home).
// • Salva com prioridade ao trocar de aba/minimizar.
// • SCHEMA 3 (migra sozinho das versões 1 e 2).
// ═══════════════════════════════════════════════════════════════
...
const GDIUser=(()=>{
  ...
  // junta o estado local com o do servidor (o mais recente vence por item)
  function mergeInto(local,remote){
    ...
    try{
      ...
      if(remote.notes)for(const k in remote.notes){
        if(touched.has('notes:'+k))continue; // alterado aqui nesta sessão: local vence
        const R=remote.notes[k]||[],L=m.notes[k]||[];
        if(R.length>L.length)m.notes[k]=R;
      }
      if(remote.last&&(!m.last||newer(remote.last.at,m.last.at)))m.last=remote.last;
  ...
  return{
    ...
    setLast:p=>mut(s=>{s.last={path:_norm(p),at:Date.now()}},['last']),
    ...
    pushHistory:(p,name)=>{const k=_norm(p);if(!k)return;mut(s=>{s.history=(s.history||[]).filter(h=>h.path!==k);s.history.unshift({path:k,name:String(name||'').slice(0,120),at:Date.now()});if(s.history.length>MAX_HISTORY)s.history.length=MAX_HISTORY;},['history'])},
...
GDIUser.ready();
// salva com prioridade ao esconder a aba (reduz perda entre dispositivos)
document.addEventListener('visibilitychange',()=>{if(document.hidden)try{GDIUser.flush()}catch(_){}});

// ═══════════════════════════════════════════════════════════════
// MÓDULO 11: retomada de vídeo/áudio
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// MÓDULO 13: Pomodoro v2.4 (★ A2: título da aba acompanha a aula)
// ═══════════════════════════════════════════════════════════════
;(function(){
  function gdiPomBoot(){
    ...
    // ★ A2: ao trocar de aula, o "título base" do Pomodoro é atualizado
...
// ═══════════════════════════════════════════════════════════════
// MÓDULO 14: EXTRAS v15
// 1: velocidade salva · atalhos · marcas na timeline · filtro
// 2: continuar de onde parou · progresso pasta · streak/horas
// 3: exportar anotações · modo revisão
// 4: senhas protegidas · C1 (1ª não assistida) · C6 (toque duplo)
//    C7 (progresso do curso) · D1 (PWA best-effort)
// ═══════════════════════════════════════════════════════════════
console.log('[GDI Extras] v15 ativo');
...
// ── 1c + 3b: marcas na timeline + modo revisão ──
...
function gdiSetupVideoExtras(wrap){
  ...
  // ── C6: toque duplo no celular = ±10s (lado direito/direito) ──
...
// ── 1d: filtro "só o que falta" na playlist ──
...
// ── 2a + 2c + C3: card home (continuar + streak + recentes) ──
function gdiHomeCard(){
  ...
  hours/=3600;
  const hist=(d&&d.history||[]).filter(h=>h.path&&h.path!==p);
...
// ── 2b + C1 + C7: progresso da pasta / 1ª não assistida / curso ──
...
function gdiUpdateProgress(){
  ...
  rows.forEach(row=>{
    // conta SOMENTE aulas de vídeo (ignora PDFs, .json, anexos…)
  ...
  if(total){
    ...
    // ── C1: abrir direto a primeira aula não assistida ──
  ...
  // ── C7: somar o progresso de todas as subpastas (sob demanda) ──
...
// ── D1: PWA best-effort (instalável; offline só se existir gdi-sw.js) ──
(function(){
  try{

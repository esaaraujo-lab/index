...
console.log('[GDI app] build v12.1-flexfix');

// ── v12.1: corrige vazamento de largura (flexbox min-width:auto) ──
// A URL de download em .gdi-dl-url-text tinha min-width:auto (não encolhia)
// e empurrava a caixa além da largura do vídeo. Trava a cadeia flex inteira
// e trunca a URL com reticências (o título completo fica no hover).
...
// ── CSS do modo estudo — v12.2: TODOS os modos DENTRO da página ──
// Foco na aula / Foco no material = mesma estrutura do Dividido.
// Sem position:fixed, sem cobrir a navbar, sem scroll próprio.
...
/* v12.2: foco apenas alterna a coluna visível — layout permanece no fluxo da página */
...
/* FM agora in-flow: painel mais alto, mas sem invadir a navbar */
...
// ── Modos de layout (v12.2: dentro da página, como o Dividido) + zoom no FM ──
...
document.querySelectorAll('.gdi-mode-btn[data-mode]').forEach(b=>{
  b.addEventListener('click',()=>{
    gdiSetMode(b.dataset.mode);
    if(b.dataset.mode==='fm'){try{window.scrollTo({top:0,behavior:'smooth'})}catch(_){window.scrollTo(0,0)}}
  });
});
...
// ─────────────────────────────────────────────────────────────
// MATERIAIS — padrão clássico: painel e 3 modos sempre presentes.
// Sem PDF → mensagem no painel. Abas temáticas quando há PDFs.
// ─────────────────────────────────────────────────────────────
...
(async function buildMaterials(){
  ...
  try{
    ...
    if(!pdfs.length){
      ...
      console.log('[GDI Materiais] nenhum PDF — mensagem exibida, layout padr\u00e3o mantido');
      return;
    }

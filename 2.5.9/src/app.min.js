...
async function loadCrossFolderPlaylist(){
  return new Promise((resolve)=>{
    ...
    requestListPath(parentPath,{password:parentPw},function(parentData){
      if(!parentData||!parentData.data||!parentData.data.files){resolve([]);return;}
      let subFolders=parentData.data.files.filter(f=>f.mimeType==="application/vnd.google-apps.folder");
      if(!subFolders.length){resolve([]);return;}
      if(subFolders.length>40)subFolders=subFolders.slice(0,40);
      ...
      subFolders.forEach(folder=>{
        ...
        requestListPath(fpath,{password:fpw},function(folderData){
...
requestListPath(folderPath,{password:folderPw},async function(h){
  ...
  if(videos.length<=1){
    ...
    if(crossVideos.length>1){
      ...
      // Cross-folder: o v\u00eddeo atual \u00e9 sempre o representante da pasta dele (s\u00f3 entra
      // quando h\u00e1 \u22641 v\u00eddeo na pasta). Localiza pela PASTA \u2014 nome \u00e9 renomeado e o
      // streamUrl \u00e9 regenerado (IV aleat\u00f3rio), ent\u00e3o nunca batem por igualdade.
      const gdiNormPath=p=>decodeURIComponent(p||'').replace(/\/+$/,'');
...
(function(){let n=0;const t=setInterval(()=>{
  ...
  if(v){clearInterval(t);
    attachResumeTracking(v,()=>playlistVideos[currentIndex]?.pageUrl||window.location.pathname);}
...
// ============================================================
// RESUME — "Continuar de onde parou" (por navegador do usuário logado)
// ============================================================
const GDIResume=(()=>{
  const KEY='gdi-resume-v1',MAX=300,MIN_SAVE=5,END_MARGIN=15;
  ...
  return{
    ...
    set(p,t,d){
      if(!p)return;
      if(d&&t>d-END_MARGIN){this.del(p);return}
      if(t<MIN_SAVE)return;
      const m=all();
      m[p]={t:Math.floor(t),d:Math.floor(d||0),at:Date.now()};
      const ks=Object.keys(m);
      if(ks.length>MAX)ks.sort((a,b)=>m[a].at-m[b].at).slice(0,ks.length-MAX).forEach(k=>delete m[k]);
      write(m);
    },
    del(p){const m=all();delete m[p];write(m)}
  };
})();
...
function attachResumeTracking(media,getKey){
  ...
  const save=()=>{const k=key();if(k)GDIResume.set(k,media.currentTime,dur())};
  ...
  function applySeek(){
    ...
    if(!sv||sv.t<GDIResume.MIN_SAVE)return;
    const d=dur(); if(d&&sv.t>d-15)return;
    try{media.currentTime=sv.t}catch(e){}
...
function showResumeToast(t,onRestart){
  ...
  c.appendChild(el);
  el.querySelector('.gdi-toast-btn').addEventListener('click',e=>{e.stopPropagation();onRestart();el.remove()});
...
function initSleepMode(type){
  ...
  if(type==='audio'){
    // Interação acorda; se continuar tocando, dorme de novo
    ...
    // Conecta ao APlayer quando estiver pronto (tenta até 5s)
    let tries=0;
    ...
    // Vídeo: SEM automação — só o botão. Vídeo terminou → acorda a tela.
    let tries=0;
    const w=setInterval(()=>{
      const v=document.querySelector('video');
...
// ============================================================
// POMODORO v2 — à prova de throttling, persiste entre reloads
// ============================================================
;(function(){
...
#gdi-pom-fab>span{position:relative;z-index:1;font-size:20px;}
#gdi-pom-fab-time{position:absolute;bottom:-13px;left:50%;transform:translateX(-50%);z-index:2;
  font-size:9px;font-weight:700;color:#f0f6fc;white-space:nowrap;letter-spacing:.04em;
  background:rgba(14,15,22,.92);padding:1px 6px;border-radius:6px;font-variant-numeric:tabular-nums;}
...
#gdi-pom-sync-state{font-size:10px;color:#8b949e;text-align:center;margin-top:6px;min-height:12px;}
...
`;
  ...
  root.innerHTML=`
    ...
    <div id="gdi-pom-panel">
      ...
      <div id="gdi-pom-cfg">
        ...
        <label class="gdi-pom-switch"><span>🔗 Sincronizar com o player</span><input id="gdi-pom-c-sync" type="checkbox" checked></label>
      </div>
    </div>`;
  document.body.appendChild(root);

  const CKEY='gdi-pom-cfg-v2', SKEY='gdi-pom-state-v2';
  let cfg=Object.assign({work:25,short:5,long:15,sessions:4,autoStart:true,syncPlayer:true},
  ...
  (function restore(){
    ...
    if(s.running&&s.endAt&&s.endAt>Date.now()){
      ...
      st.remain=Math.max(0,Math.round((s.endAt-Date.now())/1000));
      st.endAt=s.endAt;
      startLoop();
    } else if(!s.running&&s.remain!=null){
      st.total=s.total||st.total; st.remain=s.remain;
    }
  })();
  ...
  function updateUI(){
    ...
    const ft=$id('gdi-pom-fab-time');
    if(ft){ft.textContent=time;ft.style.color=warn?'#ff6b6b':'#f0f6fc';}
  ...
  function loadCfgInputs(){
    ...
    $id('gdi-pom-c-sync').checked=!!cfg.syncPlayer;
  }
  loadCfgInputs();
  ...
  $id('gdi-pom-c-sync').addEventListener('change',e=>{cfg.syncPlayer=e.target.checked;localStorage.setItem(CKEY,JSON.stringify(cfg));});

  // Sync com player: polling leve (1.5s) em vez de MutationObserver subtree
  setInterval(()=>{
    const v=document.querySelector('video');
    if(v&&!v.__pomSync){
      v.__pomSync=true;
      v.addEventListener('play', ()=>{if(cfg.syncPlayer&&!st.running)start();});
      v.addEventListener('pause',()=>{if(cfg.syncPlayer&&st.running)pause();});
      v.addEventListener('ended',()=>{if(cfg.syncPlayer&&st.running)pause();});
      const s=$id('gdi-pom-sync-state');if(s)s.textContent='🔗 Sincronizado com o player';
    }
    if(!v){const s=$id('gdi-pom-sync-state');if(s&&s.textContent)s.textContent='';}
  },1500);
  ...
  updateUI();
  console.log('[GDI Pomodoro] v2 inicializado ✓');
})();

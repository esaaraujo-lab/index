...
// ═══ M22: CENTRAL DE ESTUDOS ═══
// Meus Cursos (continuar por curso, alvo verificado) + Estatísticas
// (heatmap, horas por curso) + Meta diária (medidor de minutos) +
// Flashcards (SRS 1/7/30/90) + Modo Maratona. Só extras — nada no core.
(function(){
  ...
  // ── estado: GDIUser → resgate /userstate ──
  let rescue=null,rescueAt=0;
  ...
  // ── cursos: drive + 2 níveis de pasta ──
  ...
  function bestIn(courseKey){
    ...
    return (async()=>{
      for(const c of cands.slice(0,4)){if(await exists(c.path))return c.path;}
      return null;
    })();
  }

  // ── medidor de minutos (meta diária) ──
  ...
  // chip de meta no card Continuar (M13)
  setInterval(()=>{
...
`;document.head.appendChild(s);
  ...
  document.addEventListener('keydown',e=>{
    ...
    const k=e.key.toLowerCase();
    if(k==='c'&&panel){panel.style.display!=='none'?closePanel():openPanel();return;}
